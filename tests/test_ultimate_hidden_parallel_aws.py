...
class HiddenParallelAwsTests(unittest.TestCase):
    def test_plan_is_exact_measure_first_and_resource_bounded(self) -> None:
        ...
        self.assertEqual("ultimate-hidden-parallel-aws-v1", plan["schema"])
        self.assertEqual("7af5dec2", plan["canonical_commit"])
        self.assertEqual(3, len(plan["jobs"]))
        self.assertEqual(3, len({job["instance_id"] for job in plan["jobs"]}))
        for job in plan["jobs"]:
            ...
            self.assertIn("CPUQuota=2900%", unit)
