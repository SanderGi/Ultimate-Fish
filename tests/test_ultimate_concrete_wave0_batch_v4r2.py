...
class ConcreteWave0BatchV4R2Test(unittest.TestCase):
    def test_namespace_is_add_only_and_source_pinned(self) -> None:
        ...
        old = v4.build_document()
        old_units = {str(unit["unit"]) for unit in old["units"]}
        old_work = {str(unit["work_directory"]) for unit in old["units"]}
        old_deps = {str(unit["dependency_root"]) for unit in old["units"]}
        for unit in document["units"]:
            ...
            self.assertNotIn(str(unit["unit"]), old_units)
            self.assertNotIn(str(unit["work_directory"]), old_work)
            self.assertNotIn(str(unit["dependency_root"]), old_deps)
            self.assertEqual(
    ...
    def test_fragment_has_distinct_ids_and_old_v4_supersession(self) -> None:
        document = r2.build_document()
        ...
        self.assertTrue(all(str(job["supersedes_job_id"]).startswith(
            "concrete-wave0-batch-v4-") for job in fragment["jobs"]))

    def test_config_merge_keeps_old_v4_inactive_and_validates(self) -> None:
        document = r2.build_document()
        fragment = r2.build_supervision_jobs(document)
        ...
        old_jobs = [job for job in jobs.values()
                    if str(job["id"]).startswith("concrete-wave0-batch-v4-")
                    and not str(job["id"]).startswith("concrete-wave0-batch-v4r2-")]
        self.assertEqual(24, len(r2_jobs))
        self.assertEqual(24, len(old_jobs))
        self.assertTrue(all(not job.get("advanceable") and
                            job.get("superseded_by", "").startswith(
                                "concrete-wave0-batch-v4r2-")
                            for job in old_jobs))
