...
builder = load(
    "jg_binary_compat",
    ROOT / "tools/tablebases/build_ultimate_jester_ghost_measurement_compat_aws.py")
measurement = load(
...
class MeasurementBinaryCompatibilityTest(unittest.TestCase):
    ...
    def test_committed_loader_patch_is_exactly_allowlisted(self):
        ...
        patches = []
        for old_relative, new_relative, old_sha, new_sha in expected:
            old = subprocess.check_output(
                ["git", "show",
                 f"{builder.OLD_BUNDLE_COMMIT}:{old_relative}"],
                cwd=ROOT)
