...
class StageV4Test(unittest.TestCase):
    ...
    def test_source_materialization_includes_bound_plan_manifest(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            (source / "tools/tablebases").mkdir(parents=True)
            (source / "src").mkdir(parents=True)
            plan = b"{\"schema\":\"ultimate-concrete-wave0-batch-plan-v4\"}\n"
            (source / "tools/tablebases/ultimate_concrete_wave0_batch_v4.json").write_bytes(plan)
            ...
            stage.materialize_source_root(
                ...
                extra_hashes={
                    "tools/tablebases/ultimate_concrete_wave0_batch_v4.json": digest(plan)})
            self.assertEqual(plan,
                             (target / "tools/tablebases/ultimate_concrete_wave0_batch_v4.json").read_bytes())
