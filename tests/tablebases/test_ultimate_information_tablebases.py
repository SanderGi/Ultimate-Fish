...
class InformationTablebaseSchemaTests(unittest.TestCase):
    ...
    def test_inventory_is_exactly_all_stored_jester_or_ghost_rows(self):
        ...
        for record in records:
            ...
            self.assertIn(f"`{record['filename']}`",
                          (ROOT / "tablebases" / "README.md").read_text())

    def test_solver_inventory_classifies_all_hidden_classes_exactly_once(self):
        ...
        self.assertEqual(len(supported), 88)
    ...
    def test_primary_jester_transitive_probe_dependencies_are_complete(self):
        forcing = {
            ...
            "dragon": "kdragonk.uftb", "pawn": "kpawnk.uftb",
            "berserker": "kberserkerk.uftb", "penguin": "kpenguink.uftb",
            "sniper": "ksniperk.uftb", "prince": "kprincek.uftb",
        }
        ...
        certified_dependencies = {
            entry.filename for entry in ledger.entries(
                (ROOT / "tablebases" / "README.md").read_text())
            if entry.filename and entry.status == "certified"
        }
        for filename in (*info.PRIMARY_JESTER_FILENAMES,
            ...
            for dependency in dependencies:
                if dependency not in {"kjesterqueenk.uftb",
                                      "kjesterkqueen.uftb"}:
                    self.assertIn(dependency, certified_dependencies)
        ...
        for filename in ("kghostfishermank.uftb",
            ...
            self.assertEqual(info.solver_sidecar_dependencies(filename),
                             ("kghostk.ufgm",))
            self.assertEqual(info.solver_concrete_dependencies(filename), ())
        for filename in ("kghostmagek.uftb", "kghostkmage.uftb"):
            self.assertEqual(info.solver_sidecar_dependencies(filename),
                             ("kghostk.ufgm",))
            self.assertEqual(info.solver_concrete_dependencies(filename), ())
        for filename in ("kghostparasitek.uftb", "kghostkparasite.uftb"):
            self.assertEqual(info.solver_sidecar_dependencies(filename),
                             ("kghostk.ufgm",))
            self.assertEqual(info.solver_concrete_dependencies(filename),
                             ("kparasitek.uftb",))
    ...
    def test_solver_fingerprints_are_isolated_by_implementation_domain(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertNotEqual(after_ghost["mage-ghost-same"],
                                before["mage-ghost-same"])
            self.assertNotEqual(after_ghost["mage-ghost-opposing"],
                                before["mage-ghost-opposing"])
            self.assertNotEqual(after_ghost["parasite-ghost-same"],
            ...
            probe_source = (source_root / "src" / "ultimate" /
                            "tablebases" / "tablebase_probe.cpp")
            ...
            mage_relative = info.MAGE_GHOST_SOLVER_SOURCES[-1].relative_to(ROOT)
            mage_source = source_root / mage_relative
            mage_source.write_bytes(mage_source.read_bytes() +
                                    b"\n// mage-only drift\n")
            after_mage = {
                domain: info.solver_model_fingerprint(filename,
                                                      root=source_root)
            ...
            for domain in representatives:
                if domain in {"mage-ghost-same", "mage-ghost-opposing"}:
                    self.assertNotEqual(after_mage[domain],
                                        after_reciprocal[domain])
                else:
                    self.assertEqual(after_mage[domain],
                                     after_reciprocal[domain])

            parasite_relative = (
                info.PARASITE_GHOST_SOLVER_SOURCES[-1].relative_to(ROOT))
            parasite_source = source_root / parasite_relative
            parasite_source.write_bytes(
                parasite_source.read_bytes() + b"\n// parasite-only drift\n")
            after_parasite = {
                domain: info.solver_model_fingerprint(filename,
                                                      root=source_root)
                for domain, filename in representatives.items()
            ...
            for domain in representatives:
                if domain in {"parasite-ghost-same",
                    ...
                    self.assertNotEqual(after_parasite[domain],
                                        after_mage[domain])
                else:
        ...
        self.assertEqual(
            ...
            "96bc2c8433292a52cab0b8f193fbb911a1424d77dfd94332594ebec20c243404")
        self.assertEqual(
            info.solver_model_fingerprint("kghostghostk.uftb"),
            "c792ea8545a166b0e37ea295dbab1fcfb95bc3b2ca7c79d134f53ce2f81ac73f")
        self.assertEqual(
            info.solver_model_fingerprint("kghostmagek.uftb"),
            "25fa6c55cf21f281e55940a1792ee3aa6a78922d30c2daac89002f6131719787")
        self.assertEqual(
            info.solver_model_fingerprint("kghostkmage.uftb"),
            "d56d834caa4e41bb1c78d3c5de1c95fadafd7f7f96dc41dd35f8fa3d7f2b2b85")
        self.assertEqual(
            info.solver_model_fingerprint("kghostparasitek.uftb"),
            "68bb3535ab536716eb96fd87715abd9b80bad09d3320a3eee14b8e9bb20cfbe8")
        self.assertEqual(
            info.solver_model_fingerprint("kghostkparasite.uftb"),
            "58395a7dc95881ce8d6527506b300be8eed6357ee0466a6c87dae3058ae1666f")
        self.assertNotEqual(
    ...
    def test_requires_all_hidden_rows(self):
