...
from tests.test_certify_ultimate_jester_ghost_verification_aws import (
    CertificationTest,
)
...
merge = load("jg_merge", ROOT / "tools/merge_ultimate_jester_ghost_rebound_aws.py")
...
class MergeTest(unittest.TestCase):
    def fixture(self, root: Path):
        ...
        audit_module = load(
            "jg_audit_fixture",
            ROOT / "tools/certify_ultimate_jester_ghost_verification_aws.py")
        audit_module.certify(certification)
        runner = ROOT / "tools/merge_ultimate_jester_ghost_rebound_aws.py"
    ...
    def test_production_unit_is_bounded_nonrestarting_and_read_only(self):
        unit = (ROOT / "tools/ultimatefish-jg7328-merge.service").read_text()
