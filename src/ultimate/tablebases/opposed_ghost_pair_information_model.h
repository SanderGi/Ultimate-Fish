...
#ifndef ULTIMATE_OPPOSED_GHOST_PAIR_INFORMATION_MODEL_H_INCLUDED
#define ULTIMATE_OPPOSED_GHOST_PAIR_INFORMATION_MODEL_H_INCLUDED
...
inline constexpr unsigned ProductVariables =
  Position::BoardSquares * Position::BoardSquares;
inline constexpr unsigned ProductMaskWords = (ProductVariables + 63) / 64;
inline constexpr std::uint32_t RawPublicFrameCount =
  2 * Position::BoardSquares * (Position::BoardSquares - 1) *
  (1 + 2 * (Position::BoardSquares - 2) +
   (Position::BoardSquares - 2) * (Position::BoardSquares - 3));

// Physical kghostkghost source state. Ghost ownership remains color-labelled
// through the source table's White-King horizontal fold.
struct ConcreteState {
    Color side = Color::White;
    std::uint8_t whiteKing = 0;
    std::uint8_t blackKing = 0;
    std::uint8_t whiteGhost = 0;
    std::uint8_t blackGhost = 0;
...
// Kings and visible Ghosts are public. Hidden Ghost coordinates are omitted;
// colors remain public whenever a Ghost is visible.
struct PublicFrame {
    Color side = Color::White;
    std::uint8_t whiteKing = 0;
    std::uint8_t blackKing = 0;
    std::optional<std::uint8_t> visibleWhiteGhost;
    std::optional<std::uint8_t> visibleBlackGhost;
    ...
    friend bool operator==(const PublicFrame& lhs, const PublicFrame& rhs) {
        return lhs.side == rhs.side && lhs.whiteKing == rhs.whiteKing &&
               ...
               lhs.visibleWhiteGhost == rhs.visibleWhiteGhost &&
               lhs.visibleBlackGhost == rhs.visibleBlackGhost;
    }
};

// Dense, stable public-frame codec used to seed and shard the complete graph.
// It covers hidden/hidden, either one-visible, and both-visible color-labelled
// configurations without invalid overlap slots.
...
// Each player privately knows its own Ghost coordinate. Their ordered joint
// relation is kept intact; it is never replaced by independent marginals.
struct ProductWorld {
    std::uint8_t whiteGhost = 0;
    std::uint8_t blackGhost = 0;
    ...
    friend bool operator==(const ProductWorld& lhs,
        ...
        return lhs.whiteGhost == rhs.whiteGhost &&
...
struct WorldMask {
    std::array<std::uint64_t, ProductMaskWords> words{};
...
// admission. White cells group by its exact Ghost square; Black cells group
// by its exact Ghost square. No legal-dot observation is applied until asked.
...
// A hidden Ghost adjacent to the opposing King is causally impossible because
// the native reveal hook would already have made it public.
