/* Ultimate Fish exact Bomb/Ghost public-information solver. GPLv3+. */

#ifndef ULTIMATE_GHOST_BOMB_INFORMATION_SOLVER_H_INCLUDED
#define ULTIMATE_GHOST_BOMB_INFORMATION_SOLVER_H_INCLUDED
...
struct SolveOptions {
    ...
    std::string lowerBombTable;
    std::string lowerBombFullSha256;
    std::string lowerBombSourceSha256;
    std::string lowerBombModelSha256;
...
// Exhaustive over every native edge in the authenticated transition range.
// A Bomb capture must detonate immediately, so it may only reach an exact
// terminal result; any live lower-material result is a proof residual.
struct TransitionAuditCertificate {
    std::uint64_t lowerBombEdges = 0;
    std::uint64_t lowerBombOwnerForces = 0;
    std::uint64_t lowerBombObserverForces = 0;
    std::uint64_t lowerBombDraws = 0;
    std::uint64_t bombCaptureEdges = 0;
    std::uint64_t bombCaptureOwnerWins = 0;
    std::uint64_t bombCaptureObserverWins = 0;
    std::uint64_t bombCaptureDraws = 0;
...
[[nodiscard]] std::uint32_t bomb_win_witness_geometry(
  Orientation orientation);
[[nodiscard]] std::uint32_t bomb_draw_witness_geometry();
