/* Ultimate Fish exact Dragon/Ghost information CLI. GPLv3+. */
...
namespace Model = Stockfish::Ultimate::GhostDragonExact;
    ...
    try {
        ...
        const auto choose = [&](Command next) {
            if (command != Command::None)
                throw std::invalid_argument("choose one Dragon command");
            command = next;
        };
        for (int index = 1; index < argc; ++index) {
            ...
            else if (option == "--lower-dragon-sha256")
                transition.lowerDragonSha256 =
                  solve.lowerDragonFullSha256 = value();
            else if (option == "--lower-dragon-source-sha256")
                transition.lowerDragonSourceSha256 =
                  solve.lowerDragonSourceSha256 = value();
        ...
        if (command == Command::None)
            throw std::invalid_argument("missing Dragon command");
        if (command == Command::SelfTest) {
            const std::string scratch = solve.scratchPrefix.empty()
              ? "/tmp/ultimate-dragon-ghost" : solve.scratchPrefix;
            Model::exact_self_test(scratch);
            if (!solve.sourceTable.empty())
                std::cout << "dragon_ghost_normalized_source_sha256 "
            ...
            std::cout << "dragon_ghost_resource geometries "
            ...
            std::cout << "dragon_ghost_normalized_source_sha256 "
                      << Model::verify_source_normalization(
                           ...
                           solve.scratchPrefix.empty()
                             ? "/tmp/ultimate-dragon-ghost-source"
                             : solve.scratchPrefix) << '\n';
        }
        ...
        std::cerr << "Dragon/Ghost error: " << error.what() << '\n';
        return 1;
    }
}
