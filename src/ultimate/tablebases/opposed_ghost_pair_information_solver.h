/*
  Ultimate Fish - exact opposed Ghost/Ghost information solver core
  Copyright (C) 2026 Ultimate Fish contributors
  GPLv3 or later.
*/

#ifndef ULTIMATE_OPPOSED_GHOST_PAIR_INFORMATION_SOLVER_H_INCLUDED
#define ULTIMATE_OPPOSED_GHOST_PAIR_INFORMATION_SOLVER_H_INCLUDED
...
struct NodeExpansion {
    OpposedGhostPairInformation::CompleteTransitions transitions;
...
// A Bellman child is either a variable in the opposed-Ghost graph or an exact
...
struct ChildReference {
    OpposedGhostPairInformation::ChildDomain domain =
      OpposedGhostPairInformation::ChildDomain::Invalid;
    std::optional<NodeId> node;
    std::uint32_t bucket = 0;
    std::uint32_t childAtom = 0;
    OpposedGhostPairInformation::ClassifiedChild child;
...
struct ExternalForceQuery {
    ...
    std::uint32_t bucket = 0;
    std::uint32_t childAtom = 0;
    OpposedGhostPairInformation::ClassifiedChild actual;
    ...
    std::vector<OpposedGhostPairInformation::ClassifiedChild> belief;
};
...
struct LowerJesterForceQuery {
    ...
    OpposedGhostPairInformation::LowerJesterSet belief;
};

struct LowerGhostForceQuery {
    Color target = Color::White;
    OpposedGhostPairInformation::Role targetRole =
      OpposedGhostPairInformation::Role::Observer;
    OpposedGhostPairInformation::LowerGhostState actual;
    OpposedGhostPairInformation::LowerGhostImage belief;
};
...
class Arena {
   public:
    ...
    [[nodiscard]] NodeId intern(
      const OpposedGhostPairInformation::KnowledgeState& state);
    [[nodiscard]] std::optional<NodeId> find(
      const OpposedGhostPairInformation::KnowledgeState& state) const;
    ...
    [[nodiscard]] OpposedGhostPairInformation::KnowledgeState node(
      NodeId id) const;
