/*
  Ultimate Fish - exact opposed K+Ghost versus K+Ghost information model
...
namespace Stockfish::Ultimate::OpposedGhostPairInformation {
namespace {
...
void validate_side(Color side) {
    if (side != Color::White && side != Color::Black)
        throw std::invalid_argument("invalid opposed-Ghost-information side");
}
...
[[nodiscard]] std::uint8_t unrank_excluding(
  std::uint32_t rank, std::initializer_list<std::uint8_t> occupied) {
    ...
    throw std::runtime_error("opposed-Ghost-information square rank is invalid");
}
...
void validate_source(const ConcreteState& state) {
    ...
    validate_square(state.whiteGhost, "White Ghost square");
    validate_square(state.blackGhost, "Black Ghost square");
    if (!distinct({state.whiteKing, state.blackKing, state.whiteGhost,
                   state.blackGhost}))
        throw std::invalid_argument("overlapping opposed-Ghost source state");
}
...
[[nodiscard]] ConcreteState horizontal_canonical(ConcreteState state) {
    ...
    if (state.whiteKing % Position::BoardFiles >= Position::BoardFiles / 2) {
        ...
        state.whiteGhost = horizontal_reflection(state.whiteGhost);
        state.blackGhost = horizontal_reflection(state.blackGhost);
    }
    return state;
}
...
void validate_frame(const PublicFrame& frame) {
    ...
    if (frame.visibleWhiteGhost)
        validate_square(*frame.visibleWhiteGhost, "visible White Ghost square");
    if (frame.visibleBlackGhost)
        validate_square(*frame.visibleBlackGhost, "visible Black Ghost square");
    std::vector<std::uint8_t> publicSquares{frame.whiteKing, frame.blackKing};
    if (frame.visibleWhiteGhost)
        publicSquares.push_back(*frame.visibleWhiteGhost);
    if (frame.visibleBlackGhost)
        publicSquares.push_back(*frame.visibleBlackGhost);
    if (!distinct({frame.whiteKing, frame.blackKing}) ||
        std::set<std::uint8_t>(publicSquares.begin(), publicSquares.end()).size()
          != publicSquares.size())
        throw std::invalid_argument("invalid opposed-Ghost public frame");
}

void validate_world(const PublicFrame& frame, const ProductWorld& world) {
    validate_frame(frame);
    validate_square(world.whiteGhost, "product White Ghost square");
    validate_square(world.blackGhost, "product Black Ghost square");
    if (!distinct({frame.whiteKing, frame.blackKing, world.whiteGhost,
                   world.blackGhost}) ||
        (frame.visibleWhiteGhost &&
         *frame.visibleWhiteGhost != world.whiteGhost) ||
        (frame.visibleBlackGhost &&
         *frame.visibleBlackGhost != world.blackGhost))
        throw std::invalid_argument("invalid opposed-Ghost product world");
}
...
frame_key(const PublicFrame& frame) {
    return {static_cast<unsigned>(frame.side), frame.whiteKing, frame.blackKing,
      frame.visibleWhiteGhost ? *frame.visibleWhiteGhost : Squares,
      frame.visibleBlackGhost ? *frame.visibleBlackGhost : Squares,
      0};
}
...
struct LiveMaterial {
    ...
    int whiteGhost = Position::NoSquare;
    int blackGhost = Position::NoSquare;
    bool whiteGhostVisible = false;
    bool blackGhostVisible = false;
    int live = 0;
    ...
    for (int id = 0; id < position.piece_count(); ++id) {
        ...
        if (piece.type == PieceType::King && piece.color == Color::White &&
            ...
            result.blackKing = piece.square;
                 ...
                 result.whiteGhost == Position::NoSquare) {
            result.whiteGhost = piece.square;
            result.whiteGhostVisible = piece.visible;
        }
        else if (piece.type == PieceType::Ghost &&
                 piece.color == Color::Black &&
                 result.blackGhost == Position::NoSquare) {
            result.blackGhost = piece.square;
            result.blackGhostVisible = piece.visible;
        }
        else
  ...
  const std::vector<HistoryAtom>& atoms, Color player) {
    ...
    for (std::uint32_t atom = 0; atom < atoms.size(); ++atom) {
        ...
        const unsigned key = player == Color::White
                           ? world.whiteGhost : world.blackGhost;
...
[[nodiscard]] std::string common_position_key(const Position& position) {
    ...
    out << "UFOPPGHOSTVIEW1|turn=" << int(position.side_to_move())
    ...
    if (live.whiteKing != Position::NoSquare)
        out << Position::square_name(live.whiteKing);
    else
        out << '-';
    out << "|blackKing=";
    if (live.blackKing != Position::NoSquare)
        ...
        out << '-';
    out << "|whiteGhost=";
    if (live.whiteGhost == Position::NoSquare)
        out << '-';
    else if (!live.whiteGhostVisible)
        out << "hidden";
    else
        out << Position::square_name(live.whiteGhost);
    out << "|blackGhost=";
    if (live.blackGhost == Position::NoSquare)
        out << '-';
    else if (!live.blackGhostVisible)
        out << "hidden";
    else
        out << Position::square_name(live.blackGhost);
    return out.str();
}
...
[[nodiscard]] std::string common_transition_key(
  const Position& before, const Move& move, const Position& after) {
    if (move.kind == MoveKind::Pass)
        return "UFOPPGHOSTTRANS1|actor=none|kind=pass|from=-|to=-|view=" +
    ...
    const char* actorName = piece.type == PieceType::Ghost
                          ? (piece.color == Color::White
                               ? "whiteGhost" : "blackGhost")
                          : piece.color == Color::White
                          ? "whiteKing" : "blackKing";
    std::ostringstream out;
    out << "UFOPPGHOSTTRANS1|actor=" << actorName
        << "|kind=" << int(move.kind)
...
std::uint32_t encode_public_frame(const PublicFrame& frame) {
    validate_frame(frame);
    constexpr std::uint32_t Free = Squares - 2;
    constexpr std::uint32_t VisibilityCount = 1 + 2 * Free + Free * (Free - 1);
    const std::uint32_t blackRank = rank_excluding(
      frame.blackKing, {frame.whiteKing});
    std::uint32_t visibility = 0;
    if (frame.visibleWhiteGhost && frame.visibleBlackGhost) {
        const std::uint32_t whiteRank = rank_excluding(
          *frame.visibleWhiteGhost, {frame.whiteKing, frame.blackKing});
        const std::uint32_t blackGhostRank = rank_excluding(
          *frame.visibleBlackGhost,
          {frame.whiteKing, frame.blackKing, *frame.visibleWhiteGhost});
        visibility = 1 + 2 * Free + whiteRank * (Free - 1) + blackGhostRank;
    }
    else if (frame.visibleWhiteGhost)
        visibility = 1 + rank_excluding(
          *frame.visibleWhiteGhost, {frame.whiteKing, frame.blackKing});
    else if (frame.visibleBlackGhost)
        visibility = 1 + Free + rank_excluding(
          *frame.visibleBlackGhost, {frame.whiteKing, frame.blackKing});
    return (((static_cast<std::uint32_t>(frame.side) * Squares +
              frame.whiteKing) * (Squares - 1) + blackRank) *
...
PublicFrame decode_public_frame(std::uint32_t index) {
    ...
    PublicFrame frame{side, whiteKing, blackKing, {}, {}};
    if (visibility >= 1 && visibility <= Free)
        frame.visibleWhiteGhost = unrank_excluding(
          visibility - 1, {whiteKing, blackKing});
    else if (visibility > Free && visibility <= 2 * Free)
        frame.visibleBlackGhost = unrank_excluding(
          visibility - 1 - Free, {whiteKing, blackKing});
    else if (visibility > 2 * Free) {
        const std::uint32_t pair = visibility - 1 - 2 * Free;
        const std::uint32_t whiteRank = pair / (Free - 1);
        const std::uint32_t blackGhostRank = pair % (Free - 1);
        frame.visibleWhiteGhost = unrank_excluding(
          whiteRank, {whiteKing, blackKing});
        frame.visibleBlackGhost = unrank_excluding(
          blackGhostRank,
          {whiteKing, blackKing, *frame.visibleWhiteGhost});
    }
...
std::uint32_t encode_source(const ConcreteState& source) {
    ...
    const std::uint32_t whiteGhostRank = rank_excluding(
      state.whiteGhost, {state.whiteKing, state.blackKing});
    const std::uint32_t blackGhostRank = rank_excluding(
      state.blackGhost, {state.whiteKing, state.blackKing, state.whiteGhost});
    const std::uint32_t placement =
      ((((static_cast<std::uint32_t>(state.side) * (Squares / 2) + whiteRank) *
          (Squares - 1) + blackRank) *
         (Squares - 2) + whiteGhostRank) *
        (Squares - 3) + blackGhostRank);
    const std::uint32_t visibility =
      (state.whiteGhostVisible ? 2u : 0u) +
      (state.blackGhostVisible ? 1u : 0u);
...
ConcreteState decode_source(std::uint32_t index) {
    ...
    index /= 4;
    const std::uint32_t blackGhostRank = index % (Squares - 3);
    index /= Squares - 3;
    const std::uint32_t whiteGhostRank = index % (Squares - 2);
    index /= Squares - 2;
    const std::uint32_t blackRank = index % (Squares - 1);
    index /= Squares - 1;
    ...
    const std::uint8_t blackKing = unrank_excluding(
      blackRank, {whiteKing});
    const std::uint8_t whiteGhost = unrank_excluding(
      whiteGhostRank, {whiteKing, blackKing});
    const std::uint8_t blackGhost = unrank_excluding(
      blackGhostRank, {whiteKing, blackKing, whiteGhost});
    return {side, whiteKing, blackKing, whiteGhost, blackGhost,
            bool(visibility & 2), bool(visibility & 1)};
}

FramedWorld source_to_product(const ConcreteState& state) {
    validate_source(state);
    PublicFrame frame{state.side, state.whiteKing, state.blackKing, {}, {}};
    if (state.whiteGhostVisible)
        frame.visibleWhiteGhost = state.whiteGhost;
    if (state.blackGhostVisible)
        frame.visibleBlackGhost = state.blackGhost;
    const ProductWorld world{state.whiteGhost, state.blackGhost};
...
ConcreteState product_to_source(const PublicFrame& frame,
    ...
    return {frame.side, frame.whiteKing, frame.blackKing,
            world.whiteGhost, world.blackGhost,
            frame.visibleWhiteGhost.has_value(),
            frame.visibleBlackGhost.has_value()};
}
...
Position make_position(const PublicFrame& frame, const ProductWorld& world) {
    ...
    const int whiteGhost = position.add_piece(
      PieceType::Ghost, Color::White, state.whiteGhost);
    const int blackGhost = position.add_piece(
      PieceType::Ghost, Color::Black, state.blackGhost);
    if (whiteKing == Position::NoPiece || blackKing == Position::NoPiece ||
        whiteGhost == Position::NoPiece || blackGhost == Position::NoPiece)
        throw std::runtime_error("cannot construct opposed-Ghost product world");
    for (const int id : {whiteKing, blackKing, whiteGhost, blackGhost})
        position.piece(id).moved = true;
    position.piece(whiteGhost).visible = state.whiteGhostVisible;
    position.piece(blackGhost).visible = state.blackGhostVisible;
...
unsigned world_variable(const PublicFrame& frame,
                        const ProductWorld& world) {
    validate_world(frame, world);
    return unsigned(world.whiteGhost) * Squares + world.blackGhost;
}
...
std::vector<ProductWorld> geometric_worlds(const PublicFrame& frame) {
    ...
    result.reserve((frame.visibleWhiteGhost ? 1 : Squares - 2) *
                   (frame.visibleBlackGhost ? 1 : Squares - 3));
    for (std::uint8_t whiteGhost = 0; whiteGhost < Squares; ++whiteGhost) {
        if (frame.visibleWhiteGhost &&
            whiteGhost != *frame.visibleWhiteGhost)
            continue;
        for (std::uint8_t blackGhost = 0; blackGhost < Squares; ++blackGhost) {
            if (frame.visibleBlackGhost &&
                blackGhost != *frame.visibleBlackGhost)
                continue;
            ProductWorld world{whiteGhost, blackGhost};
            try {
                validate_world(frame, world);
    ...
    const std::size_t expected =
      frame.visibleWhiteGhost && frame.visibleBlackGhost ? 1 :
      frame.visibleWhiteGhost || frame.visibleBlackGhost ? Squares - 3 :
      (Squares - 2) * (Squares - 3);
    if (result.size() != expected)
        throw std::runtime_error("opposed-Ghost product cardinality residual");
    return result;
}
...
KnowledgeState fresh_state(const PublicFrame& frame) {
    ...
    if (!result)
        throw std::invalid_argument(
          "opposed-Ghost public frame has no admitted fresh world");
...
FramedWorld transform_world(const PublicFrame& frame,
    ...
    if (frame.visibleWhiteGhost)
        mapped.visibleWhiteGhost = transform_square(
          *frame.visibleWhiteGhost, transform);
    if (frame.visibleBlackGhost)
        mapped.visibleBlackGhost = transform_square(
          *frame.visibleBlackGhost, transform);
    ProductWorld mappedWorld{
      transform_square(world.whiteGhost, transform),
      transform_square(world.blackGhost, transform)};
...
std::vector<std::uint8_t> serialize_state(const KnowledgeState& state) {
    ...
    byte(state.frame.whiteKing);
    byte(state.frame.blackKing);
    byte(state.frame.visibleWhiteGhost ? 1 : 0);
    byte(state.frame.visibleWhiteGhost ? *state.frame.visibleWhiteGhost : 0xff);
    byte(state.frame.visibleBlackGhost ? 1 : 0);
    byte(state.frame.visibleBlackGhost ? *state.frame.visibleBlackGhost : 0xff);
    word(static_cast<std::uint32_t>(state.atoms.size()));
    for (const HistoryAtom& atom : state.atoms) {
        byte(atom.world.whiteGhost);
        byte(atom.world.blackGhost);
    }
...
KnowledgeState deserialize_state(const std::vector<std::uint8_t>& bytes) {
    ...
    result.frame.whiteKing = byte();
    result.frame.blackKing = byte();
    const std::uint8_t whiteVisible = byte();
    const std::uint8_t whiteSquare = byte();
    const std::uint8_t blackVisible = byte();
    const std::uint8_t blackSquare = byte();
    if (whiteVisible > 1 || (!whiteVisible && whiteSquare != 0xff) ||
        blackVisible > 1 || (!blackVisible && blackSquare != 0xff))
        throw std::invalid_argument("invalid opposed-Ghost visibility field");
    if (whiteVisible)
        result.frame.visibleWhiteGhost = whiteSquare;
    if (blackVisible)
        result.frame.visibleBlackGhost = blackSquare;
    ...
    for (std::uint32_t atom = 0; atom < atoms; ++atom) {
        const std::uint8_t whiteGhost = byte();
        const std::uint8_t blackGhost = byte();
        result.atoms.push_back({{whiteGhost, blackGhost}});
    }
...
ClassifiedChild classify_child(const Position& position) {
    ...
    if (!live.unexpected && live.live == 4 &&
        ...
        live.whiteGhost != Position::NoSquare &&
        live.blackGhost != Position::NoSquare) {
        return {ChildDomain::SameClass,
          encode_source({position.side_to_move(),
            ...
            static_cast<std::uint8_t>(live.whiteGhost),
            static_cast<std::uint8_t>(live.blackGhost),
            live.whiteGhostVisible, live.blackGhostVisible}), {}};
    }
    if (!live.unexpected && live.live == 3 &&
        ...
        (live.whiteGhost != Position::NoSquare) !=
          (live.blackGhost != Position::NoSquare)) {
        const bool whiteOwns = live.whiteGhost != Position::NoSquare;
        return {ChildDomain::LowerGhost,
          encode_lower_ghost({
            position.side_to_move() ==
                (whiteOwns ? Color::White : Color::Black)
              ? Role::GhostOwner : Role::Observer,
            static_cast<std::uint8_t>(whiteOwns
              ? live.whiteKing : live.blackKing),
            static_cast<std::uint8_t>(whiteOwns
              ? live.blackKing : live.whiteKing),
            static_cast<std::uint8_t>(whiteOwns
              ? live.whiteGhost : live.blackGhost),
            whiteOwns ? live.whiteGhostVisible : live.blackGhostVisible}), {}};
    }
...
AdmissionVerdict fresh_world_admission(const PublicFrame& frame,
    ...
    if ((!frame.visibleWhiteGhost &&
         adjacent(world.whiteGhost, frame.blackKing)) ||
        (!frame.visibleBlackGhost &&
         adjacent(world.blackGhost, frame.whiteKing)))
        return AdmissionVerdict::Reject;
