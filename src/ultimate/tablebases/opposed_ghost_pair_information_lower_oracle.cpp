...
class AuthenticatedLowerForceOracle::Impl {
   public:
    ...
    {
        // Opposed Ghost/Ghost has only K+Ghost capture children. Keeping the
        // shared interface lets the Bellman engine remain material-agnostic,
        // while deliberately not authenticating irrelevant Jester artifacts.
        load_ghost(options);
    ...
    bool force(const LowerJesterForceQuery& query) const {
        ...
        throw std::runtime_error(
          "opposed Ghost/Ghost emitted an impossible lower-Jester edge");
    }
   ...
   private:
    ...
    // Retained only so the shared, unreachable Jester loader remains a
    // compile-time checked implementation; the constructor never invokes it.
