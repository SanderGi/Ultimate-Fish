/* Ultimate Fish exact Fisherman/Ghost information CLI. GPLv3+. */
...
namespace Model = Stockfish::Ultimate::GhostFishermanExact;
    ...
    try {
        ...
        const auto choose = [&](Command next) {
            if (command != Command::None)
                throw std::invalid_argument("choose one Fisherman command");
            command = next;
        };
        ...
        if (command == Command::None)
            throw std::invalid_argument("missing Fisherman command");
        if (command == Command::SelfTest) {
            const std::string scratch = solve.scratchPrefix.empty()
              ? "/tmp/ultimate-fisherman-ghost" : solve.scratchPrefix;
            Model::exact_self_test(scratch);
            if (!solve.sourceTable.empty())
                std::cout << "fisherman_ghost_normalized_source_sha256 "
            ...
            std::cout << "fisherman_ghost_resource geometries "
            ...
            std::cout << "fisherman_ghost_normalized_source_sha256 "
                      << Model::verify_source_normalization(
                           ...
                           solve.scratchPrefix.empty()
                             ? "/tmp/ultimate-fisherman-ghost-source"
                             : solve.scratchPrefix) << '\n';
        }
            ...
            std::cout << "fisherman_ghost_certificate dual_force_residual "
        ...
        std::cerr << "Fisherman/Ghost error: " << error.what() << '\n';
        return 1;
    }
}
