/* Ultimate Fish exact Giant/Ghost information CLI. GPLv3+. */
...
namespace Model = Stockfish::Ultimate::GhostGiantExact;
    ...
    try {
        ...
        const auto choose = [&](Command next) {
            if (command != Command::None)
                throw std::invalid_argument("choose one Giant command");
            command = next;
        };
        for (int index = 1; index < argc; ++index) {
            ...
            else if (option == "--lower-giant-table")
                transition.lowerGiantTable = solve.lowerGiantTable = value();
            ...
            else if (option == "--lower-giant-sha256")
                transition.lowerGiantSha256 =
                  solve.lowerGiantFullSha256 = value();
            else if (option == "--lower-giant-source-sha256")
                transition.lowerGiantSourceSha256 =
                  solve.lowerGiantSourceSha256 = value();
            else if (option == "--lower-giant-model-sha256")
                transition.lowerGiantModelSha256 =
                  solve.lowerGiantModelSha256 = value();
        ...
        if (command == Command::None)
            throw std::invalid_argument("missing Giant command");
        if (command == Command::SelfTest) {
            const std::string scratch = solve.scratchPrefix.empty()
              ? "/tmp/ultimate-giant-ghost" : solve.scratchPrefix;
            Model::exact_self_test(scratch);
            if (!solve.sourceTable.empty())
                std::cout << "giant_ghost_normalized_source_sha256 "
            ...
            std::cout << "giant_ghost_resource geometries "
            ...
            std::cout << "giant_ghost_normalized_source_sha256 "
                      << Model::verify_source_normalization(
                           ...
                           solve.scratchPrefix.empty()
                             ? "/tmp/ultimate-giant-ghost-source"
                             : solve.scratchPrefix) << '\n';
        }
            ...
            std::cout << "giant_ghost_certificate dual_force_residual "
        ...
        std::cerr << "Giant/Ghost error: " << error.what() << '\n';
        return 1;
    }
}
