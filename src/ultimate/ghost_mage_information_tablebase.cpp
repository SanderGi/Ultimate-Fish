/* Ultimate Fish exact Mage/Ghost information CLI. GPLv3+. */
...
namespace Model = Stockfish::Ultimate::GhostMageExact;
    ...
    try {
        ...
        const auto choose = [&](Command next) {
            if (command != Command::None)
                throw std::invalid_argument("choose one Mage command");
            command = next;
        };
        ...
        if (command == Command::None)
            throw std::invalid_argument("missing Mage command");
        if (command == Command::SelfTest) {
            const std::string scratch = solve.scratchPrefix.empty()
              ? "/tmp/ultimate-mage-ghost" : solve.scratchPrefix;
            Model::exact_self_test(scratch);
            if (!solve.sourceTable.empty())
                std::cout << "mage_ghost_normalized_source_sha256 "
            ...
            std::cout << "mage_ghost_resource geometries "
            ...
            std::cout << "mage_ghost_normalized_source_sha256 "
                      << Model::verify_source_normalization(
                           ...
                           solve.scratchPrefix.empty()
                             ? "/tmp/ultimate-mage-ghost-source"
                             : solve.scratchPrefix) << '\n';
        }
            ...
            std::cout << "mage_ghost_certificate dual_force_residual "
        ...
        std::cerr << "Mage/Ghost error: " << error.what() << '\n';
        return 1;
    }
}
