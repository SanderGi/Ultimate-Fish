/* Ultimate Fish exact Fisherman/Ghost public-information solver. GPLv3+. */

#ifndef ULTIMATE_GHOST_FISHERMAN_INFORMATION_SOLVER_H_INCLUDED
#define ULTIMATE_GHOST_FISHERMAN_INFORMATION_SOLVER_H_INCLUDED
...
// Exhaustive Fisherman-specific supplement to the native/D2 transition proof.
// Pull is one atomic public relocation, never a forced continuation.  A blind
// collision with an invisible enemy Ghost removes both pieces and leaves the
// insufficient bare-King draw; every other material reduction must classify
// exactly through the generic K+Ghost or lone-Fisherman draw domains.
struct TransitionAuditCertificate {
    std::uint64_t edges = 0;
    std::uint64_t sameClassEdges = 0;
    std::uint64_t pullEdges = 0;
    std::uint64_t pullGhostEdges = 0;
    std::uint64_t pullRoyalEdges = 0;
    std::uint64_t pullLandingGhostEdges = 0;
    std::uint64_t blindCollisionEdges = 0;
    std::uint64_t lowerGhostEdges = 0;
    std::uint64_t insufficientFishermanEdges = 0;
...
[[nodiscard]] std::uint32_t pull_witness_geometry(Orientation orientation);
[[nodiscard]] std::uint32_t royal_pull_witness_geometry(
  Orientation orientation);
[[nodiscard]] std::uint32_t blind_collision_witness_geometry();
