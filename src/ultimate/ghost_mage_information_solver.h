/* Ultimate Fish exact Mage/Ghost public-information solver. GPLv3+. */

#ifndef ULTIMATE_GHOST_MAGE_INFORMATION_SOLVER_H_INCLUDED
#define ULTIMATE_GHOST_MAGE_INFORMATION_SOLVER_H_INCLUDED
...
// Exhaustive Mage-specific supplement to the native/D2 transition proof.
// Swap is one atomic public relocation, never a forced continuation. In this
// four-piece closure it targets only an allied King or (same-side) Ghost;
// there is no portal or persistent Mage state. Every material reduction must
// classify exactly through arbitrary K+Ghost or the lone-Mage native draw.
struct TransitionAuditCertificate {
    std::uint64_t schema = 0x00000031474d4655ULL;  // "UFMG1" little-endian.
    std::uint64_t edges = 0;
    std::uint64_t sameClassEdges = 0;
    std::uint64_t swapEdges = 0;
    std::uint64_t swapGhostEdges = 0;
    std::uint64_t swapRoyalEdges = 0;
    std::uint64_t swapD2Checks = 0;
    std::uint64_t actionCollisionChecks = 0;
    std::uint64_t attachmentChecks = 0;
    std::uint64_t lowerGhostEdges = 0;
    std::uint64_t insufficientMageEdges = 0;
    std::uint64_t exactTerminalEdges = 0;
    std::uint64_t forcedContinuationEdges = 0;
...
[[nodiscard]] TransitionAuditCertificate audit_transitions(
...
[[nodiscard]] std::uint32_t swap_witness_geometry(Orientation orientation);
[[nodiscard]] std::uint32_t royal_swap_witness_geometry(
  Orientation orientation);
[[nodiscard]] std::uint32_t ghost_capture_witness_geometry();
[[nodiscard]] std::uint32_t mage_capture_witness_geometry();
[[nodiscard]] std::uint32_t mage_only_draw_witness_geometry();
[[nodiscard]] bool fresh_root_admitted(
