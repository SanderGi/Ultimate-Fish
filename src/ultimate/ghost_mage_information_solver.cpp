/*
  Ultimate Fish - exact K+Ghost/Mage public-information solver
  ...
  included under a token-local Mage specialization; no frozen source byte is
  modified and the Mage model receives an independent catalog fingerprint.
*/
...
// Mage adds Swap. In this four-piece domain the auxiliary is a stable allied
// piece ID while the public D2 geometry is determined by (from,to). These
// narrowly scoped token shims extend that one frozen predicate; the complete
// ActionKey, including the piece-ID auxiliary, is certified independently.
#define Normal Normal && move.kind != MoveKind::Swap
#define auxiliary kind != MoveKind::Swap && move.auxiliary
#define Bishop Mage
#define fresh_root_public_grouping_self_test()                              \
    mage_constructor_grouping_bypass(); [[maybe_unused]] void               \
...
// The inherited constructor's grouping regression is deliberately tailored
// to Bishop+Ghost: it insists that its deterministic sample contain both a
// live and an ordinary terminal position. Mage never attacks, and an
// all-four-model Mage+Ghost position therefore has no ordinary terminal
// sample. The macro above redirects only that constructor call while retaining
// the complete inherited regression body under an unused name. The exact
// Mage-specific public-grouping bijection below runs immediately after the
// solver installs its real material adapter; no Bellman path is bypassed.
void ExternalGhostExtraFixedPoint::mage_constructor_grouping_bypass() {}

}  // namespace
}  // namespace Stockfish::Ultimate

namespace Stockfish::Ultimate::GhostMageExact {
namespace {

static_assert(sizeof(TransitionAuditCertificate) == 112);
...
constexpr char SidecarSemantics[] =
  "fresh-maximal-public-view-v2:mage-ghost-generic";
...
struct SidecarHeader {
    std::array<char, 8> magic{{'U','F','M','G','1','\0','\0','\0'}};
    ...
    std::uint32_t secondary = static_cast<std::uint32_t>(PieceType::Mage);
...
void write_overlay_header(std::ostream& output, Orientation orientation,
    ...
    if (!valid_sha(sourceSha) || !valid_sha(modelSha))
        throw std::invalid_argument("Mage overlay header SHA is invalid");
    ...
    write32(static_cast<std::uint32_t>(PieceType::Mage));
    ...
    if (!output)
        throw std::runtime_error("failed writing Mage overlay header");
}
    ...
    for (const Orientation orientation : {Orientation::Same,
        ...
        const auto word = [&](std::size_t offset) {
            ...
            if (offset + 4 > bytes.size())
                throw std::runtime_error("Mage overlay header is truncated");
        ...
        if (bytes.size() != 160 ||
            ...
            word(16) != static_cast<std::uint32_t>(PieceType::Mage) ||
            ...
            throw std::runtime_error("Mage UFIW2 header contract residual");
        ...
        if (ownerTurn != std::pair<bool, bool>{true, false} ||
            ...
            throw std::runtime_error("Mage mover-role summary residual");
    }
    std::cout << "ghost_mage_overlay_contract same Ghost/Mage/White"
                 " opposing Ghost/Mage/Black role_residual 0\n";
}

void sidecar_header_contract_self_test() {
    const SidecarHeader header;
    if (header.magic !=
          std::array<char, 8>{{'U','F','M','G','1','\0','\0','\0'}} ||
        header.version != 1 || header.headerBytes != sizeof(SidecarHeader) ||
        header.endian != Endian ||
        header.primary != static_cast<std::uint32_t>(PieceType::Ghost) ||
        header.secondary != static_cast<std::uint32_t>(PieceType::Mage) ||
        header.ghostColor != static_cast<std::uint32_t>(Color::White) ||
        header.squares != Squares || header.stateCount != StateCount ||
        header.nodeBytes != sizeof(NodeDisk) ||
        header.geometryBytes != sizeof(ExternalGeometryMeta) ||
        header.maskBytes != sizeof(ExternalMask) ||
        header.rootBytes != sizeof(ExternalRobdd::Id) || header.reserved)
        throw std::runtime_error("Mage UFMG1 header contract residual");
}

void mage_observation_self_test() {
    const auto square = [](const char* name) {
        const int value = Position::square_from_name(name);
        if (value == Position::NoSquare)
            throw std::runtime_error("invalid Mage observation square");
        return static_cast<std::uint8_t>(value);
    };
    const auto swap_to = [](const Position& position, std::uint8_t target) {
        for (const Move& move : position.legal_moves())
            if (move.kind == MoveKind::Swap && move.to == target)
                return move;
        throw std::runtime_error("Mage observation fixture lacks Swap");
    };

    // When the owner swaps its Mage with its own invisible Ghost, the two
    // pre-move worlds are publicly identical but the Mage's public endpoint
    // distinguishes the resulting observations. The observer may therefore
    // narrow to the displaced Ghost square; no hidden coordinate is leaked
    // before the action.
    const auto same = adapter_material(Orientation::Same);
    GhostPublicExtra::ConcreteState first;
    first.side = Color::White;
    first.whiteKing = square("h1");
    first.blackKing = square("h10");
    first = GhostPublicExtra::with_piece_squares(
      first, same, square("a2"), square("d3"));
    GhostPublicExtra::ConcreteState second = first;
    second = GhostPublicExtra::with_piece_squares(
      second, same, square("a2"), square("e3"));
    Position firstPosition = GhostPublicExtra::make_position(first, same);
    Position secondPosition = GhostPublicExtra::make_position(second, same);
    const DisclosureContext sameObserver{~same.ghostColor, false};
    if (view_key(firstPosition, sameObserver) !=
        view_key(secondPosition, sameObserver))
        throw std::runtime_error("Mage hidden pre-swap view leaked a square");
    const DisclosureContext sameOwner{same.ghostColor, false};
    if (decision_observation_key(firstPosition, sameOwner) ==
        decision_observation_key(secondPosition, sameOwner))
        throw std::runtime_error(
          "Mage owner legal dots omitted its hidden Ghost endpoint");
    const Move firstSwap = swap_to(firstPosition, square("d3"));
    const Move secondSwap = swap_to(secondPosition, square("e3"));
    Position firstChild = firstPosition;
    Position secondChild = secondPosition;
    Undo firstUndo, secondUndo;
    if (!firstChild.make_move(firstSwap, firstUndo) ||
        !secondChild.make_move(secondSwap, secondUndo) ||
        complete_transition_observation(firstPosition, firstSwap, firstChild,
                                        sameObserver) ==
          complete_transition_observation(secondPosition, secondSwap,
                                          secondChild, sameObserver))
        throw std::runtime_error(
          "Mage hidden-Ghost Swap observation did not partition worlds");

    // In the opposing row, swapping the observer-owned Mage and King is
    // independent of a remote invisible enemy Ghost. The non-owner's two
    // worlds and resulting observations must remain indistinguishable.
    const auto opposing = adapter_material(Orientation::Opposing);
    GhostPublicExtra::ConcreteState remoteFirst;
    remoteFirst.side = Color::Black;
    remoteFirst.whiteKing = square("h10");
    remoteFirst.blackKing = square("d3");
    remoteFirst = GhostPublicExtra::with_piece_squares(
      remoteFirst, opposing, square("a2"), square("f6"));
    GhostPublicExtra::ConcreteState remoteSecond = remoteFirst;
    remoteSecond = GhostPublicExtra::with_piece_squares(
      remoteSecond, opposing, square("a2"), square("g6"));
    Position remoteFirstPosition = GhostPublicExtra::make_position(
      remoteFirst, opposing);
    Position remoteSecondPosition = GhostPublicExtra::make_position(
      remoteSecond, opposing);
    const DisclosureContext opposingObserver{~opposing.ghostColor, false};
    if (view_key(remoteFirstPosition, opposingObserver) !=
          view_key(remoteSecondPosition, opposingObserver) ||
        decision_observation_key(remoteFirstPosition, opposingObserver) !=
          decision_observation_key(remoteSecondPosition, opposingObserver))
        throw std::runtime_error("Mage opposing hidden view leaked a square");
    const Move remoteFirstSwap = swap_to(remoteFirstPosition, square("d3"));
    const Move remoteSecondSwap = swap_to(remoteSecondPosition, square("d3"));
    Position remoteFirstChild = remoteFirstPosition;
    Position remoteSecondChild = remoteSecondPosition;
    Undo remoteFirstUndo, remoteSecondUndo;
    if (!remoteFirstChild.make_move(remoteFirstSwap, remoteFirstUndo) ||
        !remoteSecondChild.make_move(remoteSecondSwap, remoteSecondUndo) ||
        complete_transition_observation(
          remoteFirstPosition, remoteFirstSwap, remoteFirstChild,
          opposingObserver) !=
          complete_transition_observation(
            remoteSecondPosition, remoteSecondSwap, remoteSecondChild,
            opposingObserver))
        throw std::runtime_error(
          "Mage royal Swap spuriously revealed a remote enemy Ghost");
    std::cout << "ghost_mage_observation hidden_swap_partition 2"
                 " owner_dot_partition 2 remote_royal_swap_preserved 2"
                 " observer_dot_preserved 2 residual 0\n";
}
...
[[nodiscard]] std::string transition_payload_sha(
  const std::string& prefix) {
    ...
    for (const char* suffix : {".header", ".meta", ".strata", ".index",
        ...
        if (!input)
            throw std::runtime_error("missing Mage transition provenance");
        while (input) {
...
void validate_unique_node_tuples(const NodeDisk* nodes,
    ...
    for (std::uint64_t id = 2; id < count; ++id)
        if (!seen.emplace(nodes[id].variable, nodes[id].low,
            ...
            throw std::runtime_error(
              "Mage sidecar contains a duplicate ROBDD tuple");
}
...
[[nodiscard]] GhostPublicExtra::MaterialSpec adapter_material(
  Orientation orientation) {
    return {PieceType::Mage,
      ...
      orientation == Orientation::Same ? "kghostmagek" : "kghostkmage"};
}
...
class OriginalTable {
  public:
    OriginalTable(const std::string& path, Orientation orientation) {
        ...
        if (!input)
            throw std::runtime_error("cannot open Mage/Ghost source table");
        bytes_ = {std::istreambuf_iterator<char>(input),
                  std::istreambuf_iterator<char>()};
        const auto word = [&](std::size_t offset) {
            std::uint32_t value = 0;
            if (offset + 4 > bytes_.size())
                throw std::runtime_error("truncated Mage/Ghost source header");
        ...
        if (bytes_.size() < 48 ||
            ...
            word(40) != static_cast<std::uint32_t>(PieceType::Mage) ||
            word(44) != static_cast<std::uint32_t>(orientation ==
              Orientation::Opposing))
            throw std::runtime_error(
              "Mage/Ghost source header does not match its orientation");
        wdlBytes_ = (std::uint64_t(StateCount) + 3) / 4;
        if (48 + wdlBytes_ > bytes_.size())
            throw std::runtime_error("truncated Mage/Ghost WDL plane");
        Sha256 hash;
...
[[nodiscard]] bool live_material_is(const Position& position,
                                    unsigned kings, unsigned mages,
                                    unsigned ghosts) {
    unsigned live = 0, foundKings = 0, foundMages = 0, foundGhosts = 0;
    for (int id = 0; id < position.piece_count(); ++id) {
        ...
        foundMages += piece.type == PieceType::Mage;
        foundGhosts += piece.type == PieceType::Ghost;
        if (piece.host != Position::NoPiece || piece.attachmentOrder)
            return false;
    }
    return live == kings + mages + ghosts && foundKings == kings &&
           foundMages == mages && foundGhosts == ghosts;
}

[[nodiscard]] bool attachment_free(const Position& position) {
    for (int id = 0; id < position.piece_count(); ++id) {
        const PieceState& piece = position.piece(id);
        if (piece.alive &&
            (piece.host != Position::NoPiece || piece.attachmentOrder))
            return false;
    }
    return true;
}

TransitionAuditCertificate audit_mage_transitions(
  const std::string& prefix, const MaterialSpec& material) {
    ...
    if (!headerFile || header.material !=
        ...
        throw std::runtime_error("Mage transition header mismatch");
    ...
    if (!blocks)
        throw std::runtime_error("cannot audit Mage transition blocks");
    ...
    for (std::uint32_t local = 0; local < header.geometryCount; ++local) {
        ...
        if (bytes < sizeof(std::array<std::uint32_t, Squares + 1>) ||
            ...
            throw std::runtime_error("malformed Mage transition block");
        ...
        if (!blocks || offsets.front() || offsets.back() != edges.size())
            throw std::runtime_error("Mage transition read residual");
        ...
        for (std::uint8_t ghost = 0; ghost < Squares; ++ghost) {
            ...
            if (!live_material_is(position, 2, 1, 1))
                throw std::runtime_error(
                  "Mage source contains unsupported state fields");
            const std::vector<Move> moves = position.legal_moves();
            const auto fullActions = GhostPublicExtra::legal_action_keys(
              position);
            std::set<std::string> renderedActions;
            for (const Move& move : moves)
                renderedActions.insert(position.move_to_string(move));
            certificate.actionCollisionChecks += moves.size();
            certificate.residual += fullActions.size() != moves.size() ||
              renderedActions.size() != moves.size();
            if (moves.size() != offsets[ghost + 1] - offsets[ghost])
                throw std::runtime_error(
                  "Mage transition move-count residual");
            for (std::size_t ordinal = 0; ordinal < moves.size(); ++ordinal) {
                ...
                if (!child.make_move(move, undo))
                    throw std::runtime_error("Mage audit move failed");
                ++certificate.edges;
                certificate.attachmentChecks += 2;
                certificate.residual += !attachment_free(position) ||
                                        !attachment_free(child);
                ...
                if (move.kind == MoveKind::Swap) {
                    ++certificate.swapEdges;
                    const int movedVictim = child.piece_on(move.from);
                    const int movedActor = child.piece_on(move.to);
                    const bool swapResidual =
                      actor == Position::NoPiece || victim == Position::NoPiece ||
                      position.piece(actor).type != PieceType::Mage ||
                      position.piece(actor).color !=
                        position.piece(victim).color ||
                      (position.piece(victim).type != PieceType::King &&
                       position.piece(victim).type != PieceType::Ghost) ||
                      move.auxiliary != victim ||
                      movedActor != actor || movedVictim != victim ||
                      edge.domain != ExternalChildDomain::SameClass ||
                      !live_material_is(child, 2, 1, 1) ||
                      (position.piece(victim).type == PieceType::Ghost &&
                       position.piece(victim).visible !=
                         child.piece(victim).visible);
                    certificate.residual += swapResidual;
                    if (swapResidual)
                        throw std::runtime_error(
                          "Mage atomic-swap transition residual at " +
                    ...
                    if (victim != Position::NoPiece) {
                        certificate.swapGhostEdges +=
                          position.piece(victim).type == PieceType::Ghost;
                        certificate.swapRoyalEdges +=
                          position.piece(victim).type == PieceType::King;
                    }
                ...
                case ExternalChildDomain::Exact:
                    if (live_material_is(child, 2, 1, 0)) {
                        ++certificate.insufficientMageEdges;
    ...
    certificate.residual += certificate.edges !=
      ...
      certificate.insufficientMageEdges + certificate.exactTerminalEdges;
    if (certificate.residual)
        throw std::runtime_error("Mage transition audit residual");
    std::cout << "ghost_mage_transition_certificate edges "
              << certificate.edges << " same_class "
              << certificate.sameClassEdges << " swaps "
              << certificate.swapEdges << " swap_ghost "
              << certificate.swapGhostEdges << " swap_royal "
              << certificate.swapRoyalEdges << " swap_d2_checks "
              << certificate.swapD2Checks << " action_collision_checks "
              << certificate.actionCollisionChecks
              << " attachment_checks "
              << certificate.attachmentChecks << " lower_ghost "
              << certificate.lowerGhostEdges << " insufficient_mage "
              << certificate.insufficientMageEdges
...
// The frozen same-side K+extra+Ghost inherited-mask fixture assumes Black is
// the uninformed mover. In the reciprocal row Black owns the Ghost and its
// legal dots are private, so rebuild the exact production composition witness
// without ever asking White's disclosure context for Black's dots.
void mage_reciprocal_inherited_lower_mask_self_test(
  ExternalGhostExtraFixedPoint& solver) {
    ...
    if (solver.material_.ghostColor != Color::Black ||
        ...
        throw std::runtime_error(
          "Mage reciprocal inherited-mask ownership residual");
    ...
    if (privateDotQueries)
        throw std::runtime_error(
          "Mage reciprocal owner action leaked a private dot query");
    ...
    if (selected == groups.end() || selected->second.size() < 2)
        throw std::runtime_error(
          "Mage reciprocal lower-mask witness lacks a non-singleton bucket");
    ...
    for (const World& world : worlds) {
        ...
        if (world.childGeometry != worlds.front().childGeometry ||
            ...
            throw std::runtime_error(
              "Mage reciprocal lower observation mixed public children");
    }
...
NormalizedSource normalize_source(const OriginalTable& source,
    ...
    for (std::uint32_t index = 0; index < StateCount; ++index) {
        ...
        if (value < 1 || value > 3)
            throw std::runtime_error("Mage/Ghost source has invalid WDL");
    ...
    if (residual || originalCounts != normalizedCounts)
        throw std::runtime_error("Mage/Ghost source normalization residual");
    ...
    output.close();
    if (!output)
        throw std::runtime_error("failed writing normalized Mage/Ghost source");
    const std::string sha = GhostPublicExtraExact::sha256_file(path);
    std::cout << "ghost_mage_source_normalization states " << StateCount
...
[[nodiscard]] bool exact_fresh_root_admitted(
  const GhostPublicExtra::ConcreteState& state, Orientation orientation) {
    ...
    // Reverse the only two represented forced relocations. Same-side Mage can
    // swap the hidden allied Ghost beside the enemy royal; opposing Mage can
    // swap its allied (observer) King beside the hidden enemy Ghost. Neither
    // displaced piece runs its ordinary Ghost-reveal callback. The source
    // squares are uniquely recoverable because Swap exchanges the endpoints.
    if (state.ghostVisible || state.side == material.extraColor)
        return false;
    const std::uint8_t mage = GhostPublicExtra::extra_square(state, material);
    ...
    std::uint8_t predecessorMage = 0;
    std::uint8_t target = 0;
    if (orientation == Orientation::Same) {
        predecessorMage = ghost;
        target = mage;
        predecessor = GhostPublicExtra::with_piece_squares(
          predecessor, material, predecessorMage, mage);
    }
    else {
        predecessorMage = observerKing;
        target = mage;
        predecessor = GhostPublicExtra::with_piece_squares(
          predecessor, material, predecessorMage, ghost);
        if (material.ghostColor == Color::White)
            predecessor.blackKing = mage;
        else
            predecessor.whiteKing = mage;
    }
    Position before;
    try {
    ...
    for (const Move& move : before.legal_moves()) {
        if (move.kind != MoveKind::Swap || move.from != predecessorMage ||
            move.to != target)
            continue;
...
void mage_fresh_root_public_grouping_self_test(Orientation orientation) {
    ...
    std::uint32_t random = orientation == Orientation::Same
    ...
    for (std::uint32_t sample = 0; sample < Samples; ++sample) {
        ...
        if (!exact_fresh_root_admitted(original, orientation))
            continue;
        ...
        if (canonical.geometry.visible)
            ...
            // The app exposes selectable-square/destination dots, not the
            // stable piece-ID auxiliary on a Mage Swap. Match the shared
            // decision_observation_key contract without over-distinguishing
            // two native actions that paint the same marker.
            std::vector<std::pair<int, int>> markers;
            for (const Move& move : position.legal_moves())
                markers.emplace_back(move.kind == MoveKind::Pass ? -1
                                                                  : move.from,
                                     move.kind == MoveKind::Pass ? -1
                                                                  : move.to);
        ...
        if ((!insertedForward && forward->second != compactKey) ||
            ...
            throw std::runtime_error(
              "Mage fresh-root compact/public observation grouping differs");
    }
    if (!admitted || !live || live + terminal != admitted ||
        fullToCompact.size() != compactToFull.size())
        throw std::runtime_error(
          "Mage fresh-root public grouping lacks admitted/live coverage");
    std::cout << "ghost_mage_fresh_root_grouping samples " << Samples
              << " admitted " << admitted << " live " << live
              << " terminal " << terminal << " public_sets "
              << fullToCompact.size()
              << " terminal_optional 1 split_residual 0 merge_residual 0"
...
FreshSummary report_fresh_roots(
  ...
  Orientation orientation, const SolveOptions& options) {
    ...
    for (std::uint32_t index = 0; index < StateCount; ++index) {
        ...
        if (normalized.visible) {
            ...
            if (publicKey == NoIndex)
                throw std::runtime_error("Mage fresh root lacks stratum");
        ...
        if (external_mask_test(meta.terminal, actual)) {
            ...
            if (stratum == NoIndex ||
                ...
                throw std::runtime_error(
                  "Mage admitted root crosses decision stratum");
        ...
        if (owner && observer)
            throw std::runtime_error("Mage fresh root has dual force");
    ...
    if (certificate.groupingResidual || certificate.conservationResidual)
        throw std::runtime_error("Mage fresh-root certificate residual");
    ...
    output.close();
    if (!output)
        throw std::runtime_error("failed writing Mage information overlay");
    std::cout << "ghost_mage_root_conservation admitted " << admittedCount
              << " total " << StateCount
...
SolveCertificate write_sidecar(const SolveOptions& options,
    ...
    if (options.outputArbitrary.empty())
        throw std::invalid_argument("Mage solve requires UFMG1 output");
    ...
    if (!output || GhostPublicExtraExact::sha256_file(
        ...
        throw std::runtime_error("failed writing Mage arbitrary sidecar");
    struct stat status{};
    if (::stat(options.outputArbitrary.c_str(), &status) ||
        static_cast<std::uint64_t>(status.st_size) != extent)
        throw std::runtime_error("Mage arbitrary sidecar extent residual");
    ...
    rewrite.close();
    if (!rewrite)
        throw std::runtime_error("cannot finalize Mage sidecar");
...
void prove_sidecar_singletons(const ArbitrarySidecarProbe& probe,
    ...
    for (std::uint32_t geometry = 0;
        ...
        for (unsigned actual = 0; actual < Squares; ++actual) {
            ...
            if (external_mask_test(meta.terminal, actual)) {
                ...
                if (stratum == NoIndex)
                    throw std::runtime_error(
                      "Mage singleton lacks decision stratum");
    ...
    if (certificate.singletonResidual)
        throw std::runtime_error(
          "Mage arbitrary singleton reproduction residual");
}
...
void validate_mage_audit(const TransitionAuditCertificate& certificate) {
    if (certificate.schema != 0x00000031474d4655ULL ||
        certificate.residual || certificate.forcedContinuationEdges ||
        certificate.swapD2Checks != 3 * certificate.swapEdges ||
        certificate.actionCollisionChecks != certificate.edges ||
        certificate.attachmentChecks != 2 * certificate.edges ||
        certificate.edges != certificate.sameClassEdges +
          certificate.lowerGhostEdges +
          certificate.insufficientMageEdges +
          certificate.exactTerminalEdges)
        throw std::runtime_error(
          "Mage transition audit conservation residual");
}

void write_mage_marker(const TransitionOptions& options,
                            const TransitionAuditCertificate& certificate) {
    validate_mage_audit(certificate);
    ...
    if (!input || header.material != static_cast<std::uint32_t>(
        ...
        throw std::runtime_error("missing Mage transition marker");
    ...
    if (!output || ::stat((options.prefix + ".verified").c_str(), &status) ||
        ...
        throw std::runtime_error("Mage transition marker extent residual");
}

void authenticate_mage_marker(const TransitionOptions& options) {
    ...
    if (!marker || marker.peek() != std::char_traits<char>::eof() ||
        header.material != static_cast<std::uint32_t>(
          normalized_material(options.orientation).ghostColor))
        throw std::runtime_error("Mage transition marker residual");
    validate_mage_audit(certificate);
}

void certify_mage_transitions(const TransitionOptions& options) {
    ...
    const TransitionAuditCertificate certificate =
      audit_mage_transitions(options.prefix, material);
    write_mage_marker(options, certificate);
    authenticate_mage_marker(options);
}
...
void compile_transitions(const TransitionOptions& options) {
    if (options.prefix.empty() || !options.geometryCount)
        throw std::invalid_argument(
          "Mage/Ghost transition range is empty");
    ...
    certify_mage_transitions(options);
}
...
TransitionAuditCertificate audit_transitions(
  const TransitionOptions& options) {
    authenticate_mage_marker(options);
    return audit_mage_transitions(
      options.prefix, normalized_material(options.orientation));
}

ResourceEstimate resource_estimate() { return {}; }

std::uint32_t swap_witness_geometry(Orientation orientation) {
    const MaterialSpec material = normalized_material(orientation);
    const ExtraGeometryDomain domain;
    const PublicExtraGeometry raw = orientation == Orientation::Same
      ? PublicExtraGeometry{static_cast<std::uint8_t>(Color::White), 7, 9,
                            8, 0}
      : PublicExtraGeometry{static_cast<std::uint8_t>(Color::White), 19, 79,
                            8, 0};
    const std::uint8_t ghost = orientation == Orientation::Same ? 19 : 9;
    const std::uint8_t target = 19;
    Position position = make_geometry_position(raw, ghost, material);
    for (const Move& move : position.legal_moves()) {
        if (move.kind != MoveKind::Swap || move.from != raw.bishop ||
            move.to != target)
            continue;
        Position child = position;
        Undo undo;
        if (!child.make_move(move, undo) || child.has_forced_action())
            throw std::runtime_error(
              "Mage swap witness is not one atomic action");
        return domain.locate(raw).first;
    }
    throw std::runtime_error("cannot construct Mage swap witness");
}

std::uint32_t royal_swap_witness_geometry(Orientation orientation) {
    const MaterialSpec material = normalized_material(orientation);
    const ExtraGeometryDomain domain;
    const PublicExtraGeometry raw = orientation == Orientation::Same
      ? PublicExtraGeometry{static_cast<std::uint8_t>(Color::White), 19, 79,
      ...
      : PublicExtraGeometry{static_cast<std::uint8_t>(Color::White), 19, 79,
                            8, 0};
    const std::uint8_t ghost = orientation == Orientation::Same ? 50 : 9;
    Position position = make_geometry_position(raw, ghost, material);
    for (const Move& move : position.legal_moves()) {
        if (move.kind != MoveKind::Swap || move.from != raw.bishop ||
            move.to != raw.whiteKing)
            continue;
        Position child = position;
        Undo undo;
        if (!child.make_move(move, undo) || child.has_forced_action())
            throw std::runtime_error(
              "Mage royal swap witness is not one atomic action");
        return domain.locate(raw).first;
    }
    throw std::runtime_error("cannot construct Mage royal swap witness");
}
...
std::uint32_t ghost_capture_witness_geometry() {
    ...
    for (const Move& move : position.legal_moves()) {
        ...
        if (!child.make_move(move, undo) || child.game_over() ||
            ...
            throw std::runtime_error(
              "Ghost capture of Mage did not enter live KGhost");
        return domain.locate(raw).first;
    }
    throw std::runtime_error(
      "cannot construct Ghost-captures-Mage witness");
}

std::uint32_t mage_capture_witness_geometry() {
    const MaterialSpec material = normalized_material(Orientation::Same);
    const ExtraGeometryDomain domain;
    const PublicExtraGeometry raw{
      static_cast<std::uint8_t>(Color::Black), 0, 19, 11, 0};
    Position position = make_geometry_position(raw, 50, material);
    for (const Move& move : position.legal_moves()) {
        if (move.kind != MoveKind::Normal || move.from != 19 || move.to != 11)
            continue;
        Position child = position;
        ...
        if (!child.make_move(move, undo) || child.game_over() ||
            !live_material_is(child, 2, 0, 1))
            throw std::runtime_error(
              "Mage capture did not enter live KGhost");
        return domain.locate(raw).first;
    }
    throw std::runtime_error("cannot construct Mage-captured witness");
}

std::uint32_t mage_only_draw_witness_geometry() {
    const MaterialSpec material = normalized_material(Orientation::Same);
    const ExtraGeometryDomain domain;
    const PublicExtraGeometry raw{
      static_cast<std::uint8_t>(Color::Black), 0, 19, 79, 1};
    Position position = make_geometry_position(raw, 11, material);
    for (const Move& move : position.legal_moves()) {
        if (move.kind != MoveKind::Normal || move.from != 19 || move.to != 11)
            continue;
        Position child = position;
        Undo undo;
        if (!child.make_move(move, undo) ||
            !live_material_is(child, 2, 1, 0) || !child.game_over() ||
            child.winner().has_value())
            throw std::runtime_error(
              "Ghost capture did not enter lone-Mage native draw");
        return domain.locate(raw).first;
    }
    throw std::runtime_error("cannot construct lone-Mage draw witness");
}
...
SolveCertificate solve_exact(const SolveOptions& options) {
    if (options.measureIterations == 0 &&
        ...
        throw std::invalid_argument(
          "Mage exact solve requires UFIW2 and UFMG1 outputs");
    ...
    if (original.sha() != options.sourceSha256)
        throw std::runtime_error("Mage/Ghost concrete source SHA mismatch");
    ...
    if (hex_digest(concrete.sha()) != normalized.sha)
        throw std::runtime_error("normalized Mage source SHA mismatch");
    ...
    mage_fresh_root_public_grouping_self_test(options.orientation);
    if (options.orientation == Orientation::Same)
        solver.inherited_lower_mask_self_test();
    else
        mage_reciprocal_inherited_lower_mask_self_test(solver);
...
void exact_self_test(const std::string& scratchPrefix) {
    ...
    sidecar_header_contract_self_test();
    mage_observation_self_test();
    const auto square = [](const char* name) {
        const int value = Position::square_from_name(name);
        if (value == Position::NoSquare)
            throw std::runtime_error("invalid Mage admission square");
        return static_cast<std::uint8_t>(value);
    };
    for (const Orientation orientation : {Orientation::Same,
                                           Orientation::Opposing}) {
        ...
        reachable.whiteKing = orientation == Orientation::Same
                            ? square("h1") : square("h10");
        reachable.blackKing = orientation == Orientation::Same
                            ? square("b2") : square("a2");
        reachable = GhostPublicExtra::with_piece_squares(
          reachable, material,
          square("d3"),
          orientation == Orientation::Same ? square("a2") : square("b2"));
        if (GhostPublicExtra::classify_fresh_root_admission(
            ...
            throw std::runtime_error(
              "Mage reverse-swap admission witness residual");
        ...
        if (exact_fresh_root_admitted(wrongTurn, orientation))
            throw std::runtime_error(
              "Mage causal admission accepted the wrong turn");
    }
    std::cout << "ghost_mage_fresh_causal reverse_swap_same 1"
                 " reverse_swap_opposing 1 wrong_turn_rejected 2 residual 0\n";
    for (const Orientation orientation : {Orientation::Same,
                                           Orientation::Opposing}) {
        const GhostPublicExtra::MaterialSpec material =
          adapter_material(orientation);
        for (std::uint32_t index = 0; index < StateCount; ++index) {
            ...
            if (GhostPublicExtra::encode_index(original, material) != index)
                throw std::runtime_error("Mage/Ghost source codec residual");
            const FourState normalized = original_to_normalized(
              original, orientation);
            if (GhostPublicExtra::encode_index(
                  normalized_to_original(normalized, orientation), material) !=
                index)
                throw std::runtime_error("Mage/Ghost role remap residual");
        }
    }
    std::cout << "ghost_mage_exact_self_test codec_states "
...
std::string verify_source_normalization(
  ...
  const std::string& expectedSourceSha256, const std::string& scratchPrefix) {
    ...
    if (!expectedSourceSha256.empty() &&
        ...
        throw std::runtime_error(
          "Mage source-normalization input SHA mismatch");
    const NormalizedSource normalized = normalize_source(original,
      orientation, scratchPrefix + ".normalized.uftb");
    if (normalized.remapResidual)
        throw std::runtime_error("Mage source normalization residual");
    ...
    [[nodiscard]] bool query(
      ...
      GhostPublicExtra::GhostMask belief, bool owner) const {
        ...
        if (residual || geometry >= header_.geometries)
            throw std::runtime_error("Mage query geometry residual");
        if (!mapped.count() || !mapped.test(actual))
            throw std::invalid_argument(
              "Mage belief must be nonempty and contain actual");
        if (canonical.geometry.visible && mapped.count() != 1)
            throw std::invalid_argument(
              "visible Mage/Ghost query must be singleton");
  ...
  private:
    ...
    [[nodiscard]] const Value* at(std::uint64_t offset,
        ...
        if (offset > bytes_ || count > (bytes_ - offset) / sizeof(Value))
            throw std::runtime_error("Mage sidecar section overflow");
      ...
      const GhostPublicExtra::GhostMask& belief) const {
        while (root > ExternalRobdd::True) {
            if (root >= header_.nodes)
                throw std::runtime_error("Mage sidecar root out of range");
    ...
    void open_and_validate(const ProbeBindings& bindings,
        ...
        if (descriptor_ < 0)
            throw std::runtime_error("cannot open Mage sidecar");
        struct stat status{};
        if (::fstat(descriptor_, &status) || status.st_size <= 0)
            throw std::runtime_error("cannot stat Mage sidecar");
        ...
        if (data_ == MAP_FAILED) {
            data_ = nullptr;
            throw std::runtime_error("cannot mmap Mage sidecar");
        }
        if (bytes_ < sizeof(header_))
            throw std::runtime_error("truncated Mage sidecar");
    ...
    void validate(const ProbeBindings& bindings,
        ...
        if (header_.magic !=
            ...
            header_.secondary !=
              static_cast<std::uint32_t>(PieceType::Mage) ||
            ...
            throw std::runtime_error(
              "Mage arbitrary sidecar header/binding residual");
        ...
        if (restore) {
            if (GhostPublicExtraExact::sha256_file(restore->sourceTable) !=
                ...
                GhostPublicExtraExact::sha256_file(
                  ...
                  bindings.lowerGhostSidecarSha256)
                throw std::runtime_error(
                  "Mage strict-restore dependency mismatch");
            ...
            if (normalized.sha !=
                ...
                throw std::runtime_error(
                  "Mage normalized-source restore mismatch");
            std::size_t component = 0;
        ...
        for (std::uint64_t id = 0; id < header_.nodes; ++id) {
            ...
            if (id <= 1) {
                if (node.variable != Squares || node.low != id ||
                    node.high != id)
                    throw std::runtime_error(
                      "Mage sidecar terminal tuple residual");
            }
                     ...
                     (node.high > 1 && nodes()[node.high].variable <=
                                       node.variable))
                throw std::runtime_error(
                  "Mage sidecar ROBDD structural residual");
        }
        if (restore)
        ...
        for (std::uint64_t id = 0; id < header_.ownerRoots; ++id)
            if (owner_roots()[id] >= header_.nodes ||
                ...
                throw std::runtime_error("Mage owner-root residual");
        for (std::uint64_t id = 0; id < header_.strata; ++id)
            if (observer_roots()[id] >= header_.nodes)
                throw std::runtime_error("Mage observer-root residual");
    }
