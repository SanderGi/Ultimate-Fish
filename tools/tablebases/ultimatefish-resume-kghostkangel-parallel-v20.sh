...
readonly binary=${build_root}/ultimate_ghost_ordinary_information_tablebase-angel-opposed-v20
readonly old_root=/mnt/ultimatefish/ghost-angel-v9-159eea06/work-opposing
readonly work=${build_root}/kghostkangel-parallel-v21
readonly old_scratch=${old_root}/work/solve/kghostkangel
readonly scratch=${work}/work/solve/kghostkangel
readonly old_transition=${old_root}/work/transitions/kghostkangel
readonly transition=${work}/work/transitions/kghostkangel
...
if [[ ! -f "${scratch}.checkpoint-cloned" ]]; then
  ...
  printf '%s\n' 'iteration=15' 'bdd_slot=b' 'current_slot=next' \
...
if [[ ! -f "${transition}.isolated-clone" ]]; then
  ...
  python3 "${rebind}" "${old_transition}" "${transition}" \
    ...
    --old-model-sha256 1580faba5340a934b6efb8e5ddb1c5f947aab723630ce1e2eae19f8d8feedcb4 \
    --new-model-sha256 8a69f54a02be837ee83729fdd00a5fec6b149242cc58a126bd6d5f361621341c \
...
exec "${binary}" --solve --orientation opposing \
  ...
  --output "${work}/work/results/kghostkangel.ufiw" \
  --output-arbitrary "${work}/work/results/kghostkangel.ufgd" \
  ...
  --source-sha256 ad4be1cce6d7d5911fb91cf2bf5c3274d5eafcde23240a22c5b3dfdfa2507d9c \
  --model-sha256 8a69f54a02be837ee83729fdd00a5fec6b149242cc58a126bd6d5f361621341c \
  --observation-sha256 6263896741c27752f1510b9ed34d27c561392767b682c2917e7d6bdb5c8fd11b \
