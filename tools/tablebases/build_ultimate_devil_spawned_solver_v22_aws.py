#!/usr/bin/env python3
"""Build, preserve, and restore-check the eight-bit Devil v22 binary."""
...
SEMANTICS = (
    ...
    "stripe-aligned-packed-hash-eight-bit-exact-fingerprint-filter-"
    "90-percent-load-independent-fail-closed-hash-capacity-"
    "byte-aligned-phase-advised-key-io-v22"
)
BUILD_CPUS = "12"
...
def build(instance: str) -> str:
    ...
    tree = f"{root}/source"
    binary = f"{root}/ultimate_tablebase-devil-spawned-v22"
    commands = [
        "set -euo pipefail",
        ...
        "reverse_test=$(mktemp -d /mnt/ultimatefish/devil-v22-reverse.XXXXXX)",
        "migration_test=$(mktemp -d /mnt/ultimatefish/devil-v22-migrate.XXXXXX)",
        ...
        "binary_key=sources/binaries/devil-spawned-solver-v22/sha256/"
        "$binary_sha/ultimate_tablebase-devil-spawned-v22",
        ...
        "restore=$(mktemp /mnt/ultimatefish/devil-v22-binary.XXXXXX)",
        ...
        "\"$binary_sha\"",
        "printf '%s\\n' DEVIL_V22_BINARY_PRESERVED sha256=\"$binary_sha\" "
