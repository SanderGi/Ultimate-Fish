#!/usr/bin/env python3
"""Build, preserve, and restore-check the parallel-root Devil v12 binary."""
...
SOURCE_SHA256 = "826f6c86ba32bad33a1b197b91fbf0f70e936dc62cc11d844d0b0dfd9ee89537"
SOURCE_VERSION = "cVreVCH42Td2TcMOWz2V2GK7oDkMohxz"
SOURCE_KEY = (
    f"sources/bundles/devil-spawned-solver-v12/sha256/{SOURCE_SHA256}/"
    "ultimatefish-devil-spawned-solver-v12-source.tar"
)
SEMANTICS = (
...
def build(instance: str) -> str:
    root = f"/mnt/ultimatefish/devil-spawned-solver-v12-{SOURCE_SHA256[:8]}"
    source = f"{root}/source.tar"
    tree = f"{root}/source"
    binary = f"{root}/ultimate_tablebase-devil-spawned-v12"
    compile_command = (
    ...
    commands = [
        ...
        "binary_key=sources/binaries/devil-spawned-solver-v12/sha256/"
        "$binary_sha/ultimate_tablebase-devil-spawned-v12",
        ...
        "restore=$(mktemp /mnt/ultimatefish/devil-v12-binary.XXXXXX)",
        "trap 'rm -f \"$restore\"' EXIT",
        f"aws s3api get-object --region {base.REGION} --bucket {base.BUCKET} "
        "--key \"$binary_key\" --version-id \"$version\" "
        "\"$restore\" >/dev/null",
        "test \"$(sha256sum \"$restore\" | cut -d ' ' -f1)\" = \"$binary_sha\"",
        "printf '%s\\n' DEVIL_V12_BINARY_PRESERVED sha256=\"$binary_sha\" "
