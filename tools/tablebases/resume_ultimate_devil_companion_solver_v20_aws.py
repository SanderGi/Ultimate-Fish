#!/usr/bin/env python3
"""Resume retained Devil checkpoints with phase-advised v20 key I/O."""
...
SOURCE_SHA256 = "ae5169e3293dad81853c36287ed08c21e12805ecb1b6fcb0d66fd00674dcc60b"
SOURCE_VERSION = "qcU70r2ldOxWFmBPHTiEhIFF7xZX.L54"
SOURCE_KEY = (
    f"sources/bundles/devil-spawned-solver-v20/sha256/{SOURCE_SHA256}/"
    "ultimatefish-devil-spawned-solver-v20-source.tar"
)
BINARY_SHA256 = "77eb2637a04e797b052527dd9308f8983a9cc180d80ae143bd306ae3a96f658c"
BINARY_VERSION = "_WM.KLlc_HW9GAEVRFN_7h9ye2yoZAwY"
BINARY_KEY = (
    f"sources/binaries/devil-spawned-solver-v20/sha256/{BINARY_SHA256}/"
    "ultimate_tablebase-devil-spawned-v20"
)
ROOT = f"/mnt/ultimatefish/devil-spawned-solver-v20-{SOURCE_SHA256[:8]}"
MEMORY_MAX = v19.MEMORY_MAX
...
def stage_commands() -> list[str]:
    ...
    binary = f"{ROOT}/ultimate_tablebase-devil-spawned-v20"
    return [
...
def names(piece: str, opposed: bool, square: int,
    ...
    unit = f"ultimatefish-devil-companion-v20-{orientation}-{piece}-square-{square}"
    log = (f"{volume.rstrip('/')}/devil-companion-v15-34aa7f01/logs/"
           f"{orientation}-{piece}-square-{square}-v20.log")
    old_unit = f"ultimatefish-devil-companion-v19-{orientation}-{piece}-square-{square}"
...
def resume(instance: str, piece: str, opposed: bool, square: int,
    ...
    if (not cpus or limit <= 0 or limit >= 2**63 or
        ...
        raise ValueError("invalid v20 Devil resource")
    ...
    if reviewed_floor is not None and hash_capacity < reviewed_floor:
        raise ValueError(
            f"reviewed v20 hash capacity for {piece} square {square} is "
