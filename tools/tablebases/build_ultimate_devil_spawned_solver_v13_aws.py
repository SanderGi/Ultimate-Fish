#!/usr/bin/env python3
"""Build, preserve, and restore-check the packed-key Devil v13 binary."""
...
SOURCE_SHA256 = "7ecf1254fb3a349c77480b0b945320a26ccaca9c0cf5a35519e352c41358d911"
SOURCE_VERSION = "62_BTTbiHptflCYIklHJfckNLtFxlwJV"
SOURCE_KEY = (
    f"sources/bundles/devil-spawned-solver-v13/sha256/{SOURCE_SHA256}/"
    "ultimatefish-devil-spawned-solver-v13-source.tar"
)
SEMANTICS = (
...
def build(instance: str) -> str:
    root = f"/mnt/ultimatefish/devil-spawned-solver-v13-{SOURCE_SHA256[:8]}"
    source = f"{root}/source.tar"
    tree = f"{root}/source"
    binary = f"{root}/ultimate_tablebase-devil-spawned-v13"
    compile_command = (
    ...
    commands = [
        ...
        "binary_key=sources/binaries/devil-spawned-solver-v13/sha256/"
        "$binary_sha/ultimate_tablebase-devil-spawned-v13",
        ...
        "restore=$(mktemp /mnt/ultimatefish/devil-v13-binary.XXXXXX)",
        "trap 'rm -f \"$restore\"' EXIT",
        f"aws s3api get-object --region {base.REGION} --bucket {base.BUCKET} "
        "--key \"$binary_key\" --version-id \"$version\" "
        "\"$restore\" >/dev/null",
        "test \"$(sha256sum \"$restore\" | cut -d ' ' -f1)\" = \"$binary_sha\"",
        "printf '%s\\n' DEVIL_V13_BINARY_PRESERVED sha256=\"$binary_sha\" "
