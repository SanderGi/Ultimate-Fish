...
def source_candidate_root(
    candidate: Path,
    plan_relative: str = "tools/tablebases/ultimate_concrete_wave0_batch_v4.json",
) -> Path:
    marker = _find_unique_relative(candidate, plan_relative)
    root = marker
    for _ in Path(plan_relative).parts:
        root = root.parent
    return root
...
def materialize_source_root(target: Path, candidate: Path,
                            ...
                            plan_relative: str = "tools/tablebases/ultimate_concrete_wave0_batch_v4.json") -> None:
