#!/usr/bin/env python3
"""Resume one stopped Devil campaign lane as disjoint shard lanes."""
...
def main() -> None:
    ...
    parser.add_argument("--prior-campaign-log", type=Path, required=True)
    parser.add_argument("--prior-campaign-log-sha256", required=True)
    parser.add_argument("--campaign-lane", type=int, choices=(0, 1),
                        required=True)
    ...
    if (base.sha256_path(args.plan) != args.plan_sha256 or
            ...
            base.sha256_path(args.prior_base_log) !=
            args.prior_base_log_sha256 or
            base.sha256_path(args.prior_campaign_log) !=
            args.prior_campaign_log_sha256):
        raise RuntimeError("Devil campaign-shard provenance residual")
    ...
    if (workers != 133 or not any(
        ...
        raise RuntimeError("Devil campaign-shard plan residual")
    campaigns = plan["pair_campaigns"]
    classes = {str(campaign["id"]) for campaign in campaigns}
    if len(classes) != len(campaigns):
        raise RuntimeError("duplicate Devil campaign-shard class")

    completed, worker_complete = base.completed_tasks(
        args.prior_base_log, args.source_worker, workers, classes)
    if worker_complete:
        return
    campaign_completed, _ = base.completed_tasks(
        args.prior_campaign_log, args.source_worker, workers, classes)
    completed.update(campaign_completed)
    args.log.parent.mkdir(parents=True, exist_ok=True)
    lane_completed, _ = base.completed_tasks(
        args.log, args.source_worker, workers, classes)
    completed.update(lane_completed)
    marker = (f"DEVIL_CAMPAIGN_SHARD_COMPLETE source_worker={args.source_worker} "
              f"campaign_lane={args.campaign_lane} "
              f"shard_lane={args.shard_lane} residual=0")
    if args.log.exists():
