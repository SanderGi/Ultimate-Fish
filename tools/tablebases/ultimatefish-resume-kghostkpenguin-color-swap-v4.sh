#!/usr/bin/env bash
# Reverify the retained opposed Ghost/Penguin iteration-82 fixed point after
# swapping Penguin's physical White/Black king freeze bits together with the
# color-swapped Ghost-primary source geometry.  The v3 dominance verifier
# proved that leaving those bits untouched bound substates 1/2 and 5/6 to the
# wrong concrete WDL plane.
set -euo pipefail

readonly bucket=ultimatefish-info-20260808-a4e679c6-831688117652
readonly source_sha=cb96a41474a62fe48f5d3464a78f42c2a5ccbd3977aeb2d79bbafb082bdff94a
readonly source_version=kQIv_4q0A7hy0jfcC2.3MqAmXcgNwVgd
readonly source_key="sources/bundles/ghost-penguin-color-swap-v4/sha256/${source_sha}/ultimatefish-ghost-penguin-color-swap-v4-source.tar"
...
readonly old_model_sha=c62fb45f7b5e732714b38b883ac30906d0bc0f9ac814e1646c339472c8714959
...
readonly old_prefix="${root}/work/transitions/kghostkpenguin-singleton-dominance-v3"
readonly new_prefix="${root}/work/transitions/kghostkpenguin-color-swap-v4"
readonly log="${root}/work/logs/color-swap-v4.log"
...
grep -Fq 'ghost_extra_singleton_domain checked 88462144 excluded_adjacent_kings 8316320 residual 12793334' \
  "${root}/work/logs/resume-after-stop-v1.log"
grep -Fq 'ghost_extra_singleton_domain checked 88462144 excluded_adjacent_kings 0 information_differences 4928538 dominance_residual 3894228' \
...
readonly binary_key="sources/binaries/ghost-penguin-color-swap-v4/sha256/${binary_sha}/ultimate_ghost_penguin_color_swap_v4_linux"
...
python3 "${tool}" "${old_prefix}" "${new_prefix}" \
  ...
  --manifest "${root}/work/penguin-color-swap-v4-transition-rebind.json" \
  >"${source_root}/rebind.log"

{
  echo "penguin_ghost_color_swap_v4 source_sha256 ${source_sha} source_version ${source_version} binary_sha256 ${binary_sha} binary_version ${binary_version} model_sha256 ${model_sha} iteration 82 current_slot current bdd_slot b checkpoint_preserved 1 transition_payload_changed 0"
  cd "${root}"
  taskset -c 2 "${binary}" --solve \
    --orientation opposing \
    --transition-prefix work/transitions/kghostkpenguin-color-swap-v4 \
  ...
  echo 'penguin_ghost_color_swap_v4 complete 1'
} >>"${log}" 2>&1
