#!/usr/bin/env python3
"""Build, test, version-preserve, and restore-check Ghost v10 binaries."""
...
SOURCE_SHA256 = "77f040a01a8494a8927b09a9c8a2eff989a848973cba97c56ed9c298cf88422b"
SOURCE_VERSION = "O710rZYCJRoElnT2oKNgKnfaCBS07Zdk"
SOURCE_KEY = (
    f"sources/bundles/ghost-parallel-v10/sha256/{SOURCE_SHA256}/"
    "ultimatefish-ghost-parallel-v10-source.tar"
)
SEMANTICS = "ghost-exact-resumable-expanded-unique-index-v10"
GENERATION = "v10"
EXTRA_DEFINES = ""
PRIMARY_DEFINE = "-DULTIMATE_GHOST_ORDINARY_EXTRA_PRIMARY"
PIECES = {
    "checker": "Checker",
    "ninja": "Ninja",
    "prince": "Prince",
...
def build(instance: str, piece: str) -> str:
    macro = PIECES[piece]
    root = f"/mnt/ultimatefish/ghost-parallel-{GENERATION}-{SOURCE_SHA256[:8]}"
    source = f"{root}/source.tar"
    tree = f"{root}/source"
    binary = f"{root}/ultimate_ghost_ordinary_information_tablebase-{piece}-{GENERATION}"
    compile_sources = (
    ...
    commands = [
        ...
        ("clang++ -std=c++17 -O3 -DNDEBUG -Wall -Wextra -Wpedantic "
         ...
         PRIMARY_DEFINE + " " + EXTRA_DEFINES + " " + compile_sources +
        ...
        f"binary_key=sources/binaries/ghost-parallel-{GENERATION}/{piece}/sha256/"
        f"$binary_sha/ultimate_ghost_ordinary_information_tablebase-{piece}-{GENERATION}",
        ...
        "\"$binary_sha\"",
        f"printf '%s\\n' GHOST_PARALLEL_{GENERATION.upper()}_BINARY_PRESERVED "
