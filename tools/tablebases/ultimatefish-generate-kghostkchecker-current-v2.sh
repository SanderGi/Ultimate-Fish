#!/usr/bin/env bash
# Generate the opposed concrete oracle in the Ghost-primary physical ordering
# required by the retained kghostkchecker information graph.  v1 generated the
# color-equivalent Checker-primary ordering and was correctly rejected by the
# packed-header normalization gate.
set -euo pipefail

readonly root=/mnt/ultimatefish/checker-concrete-current-v2
...
test ! -e "${root}/tablebases/kghostkchecker-current-v2.uftb"

exec >>"${root}/logs/generate-current-v2.log" 2>&1
echo 'checker_concrete_current_v2 workers 56 source_semantics current_checker_capture ghost_primary 1 checkpoint_preserved 1'
cd "${root}"
"${binary}" \
  --piece ghost --piece2 checker --opposing \
  --disk-backed --workers 56 --checkpoint-every 2000000 \
  --output tablebases/kghostkchecker-current-v2.uftb \
  --checkpoint work/kghostkchecker-current-v2.checkpoint
sha256sum tablebases/kghostkchecker-current-v2.uftb
echo 'checker_concrete_current_v2 complete 1'
