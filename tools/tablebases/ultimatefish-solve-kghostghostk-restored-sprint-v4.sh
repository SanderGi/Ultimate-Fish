#!/usr/bin/env bash
# Solve the exact restored same-side Ghost/Ghost graph on the freed sprint host.
# The transition archive is immutable and complete; this stage never rebuilds it.
set -euo pipefail
...
test "$(df --output=avail -B1 "${root}" | tail -1)" -ge 380000000000
