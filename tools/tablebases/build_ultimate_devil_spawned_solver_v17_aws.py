#!/usr/bin/env python3
"""Build, preserve, and restore-check the fingerprint-filtered Devil v17 binary."""
...
SEMANTICS = (
    ...
    "stripe-aligned-packed-hash-four-bit-exact-fingerprint-filter-v17"
)
...
def build(instance: str) -> str:
    ...
    commands = [
        ...
        "reverse_test=$(mktemp -d /mnt/ultimatefish/devil-v17-reverse.XXXXXX)",
        "migration_test=$(mktemp -d /mnt/ultimatefish/devil-v17-migrate.XXXXXX)",
        f"{shlex.quote(binary)} --devil-reverse-self-test \"$reverse_test\"",
        ...
        "binary_key=sources/binaries/devil-spawned-solver-v17/sha256/"
        "$binary_sha/ultimate_tablebase-devil-spawned-v17",
        ...
        "restore=$(mktemp /mnt/ultimatefish/devil-v17-binary.XXXXXX)",
        ...
        "\"$binary_sha\"",
        "printf '%s\\n' DEVIL_V17_BINARY_PRESERVED sha256=\"$binary_sha\" "
