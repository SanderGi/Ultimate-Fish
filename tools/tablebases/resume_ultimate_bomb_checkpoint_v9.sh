...
# External ROBDD node arenas are fixed-capacity sparse files.  Their append-only
# prefix can be extended safely.  The unique index is disposable but its hash
# mask depends on the slot count, so retain/truncate it at the old size: the
# solver then detects the size mismatch and rebuilds a correct 2^32-slot index
# in parallel from the authoritative dense node prefix.
for slot in a b; do
...
exec >>"${log}" 2>&1
echo "bomb_ghost_opposed_resume_v15 checkpoint_archive_sha256 918b1122428a29b537450bb37c669ea3f9651bacdeb89733d2822b777689b1f1 checkpoint_archive_version .j1Dy1Ydu8FfX2ma7LJd_SDjldO24PB7 binary_sha256 7e66fa2b0a7c42342dbf834c86053a727ff98129965dadb105110d6cd4e74967 completed_iteration 11 current_slot next bdd_slot b workers 16 max_nodes 3000000000 unique_slots 4294967296"
cd "${root}"
common=(
...
echo 'bomb_ghost_opposed_resume_v15 complete 1'
