#!/usr/bin/env bash
# Find the first residual remaining after the adjacent-King padding exclusion.
set -euo pipefail
...
diagnostic_prefix=${root}/work/transitions/kpawnghostk-diagnostic-v9
scratch=work/diagnostic-v9/kpawnghostk
log=${root}/work/logs/move-count-diagnostic-v9.log

test -x "${binary}"
test ! -e "${diagnostic_prefix}.header"
test ! -e "${root}/work/diagnostic-v9"
test ! -e "${log}"
for suffix in header meta strata index blocks verified; do
  cp --reflink=always "${source_prefix}.${suffix}" \
    "${diagnostic_prefix}.${suffix}"
done
install -d -m 0755 "${root}/work/diagnostic-v9"

cd "${root}"
set +e
"${binary}" --solve \
  --orientation same \
  --transition-prefix work/transitions/kpawnghostk-diagnostic-v9 \
  ...
  --scratch "${scratch}" \
  --output work/diagnostic-v9/kpawnghostk.ufiw \
  --output-arbitrary work/diagnostic-v9/kpawnghostk.ufgd \
