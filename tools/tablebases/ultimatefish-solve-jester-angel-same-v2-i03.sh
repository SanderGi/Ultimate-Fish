#!/usr/bin/env bash
# Solve the legal-dot-corrected public-information overlay for same-team Jester/Angel.
set -euo pipefail
...
work=/mnt/ultimatefish/jester-angel-info-v2/same
...
test -s "$output"
echo "JESTER_ANGEL_SAME_INFORMATION_V2_OK output=$(sha256sum "$output" | cut -d' ' -f1) bytes=$(stat -c %s "$output")"
