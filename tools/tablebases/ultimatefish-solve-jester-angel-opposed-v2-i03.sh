#!/usr/bin/env bash
# Solve the legal-dot-corrected public-information overlay for opposed Jester/Angel.
set -euo pipefail

binary=/mnt/ultimatefish/jester-angel-v2-stage-a76ecfa9/binary/ultimate_tablebase
...
work=/mnt/ultimatefish-penguin/jester-angel-info-v2/opposed
...
test "$(sha256sum "$binary" | cut -d' ' -f1)" = \
  6290e46bb86d5c1d35efb1731ab9d24314325e0dab6d47b31382f59c7610e359
...
test -s "$output"
echo "JESTER_ANGEL_OPPOSED_INFORMATION_V2_OK output=$(sha256sum "$output" | cut -d' ' -f1) bytes=$(stat -c %s "$output")"
