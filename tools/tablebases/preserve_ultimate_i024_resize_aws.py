#!/usr/bin/env python3
"""Quiesce and authenticate i-024 checkpoints before a resize."""
...
ROOT = "/mnt/checkpoint-migrate/resize-i024-v1"
UNITS = (
    "ultimatefish-info-kghostkchecker-compositional-v6.service",
    "ultimatefish-info-substate-v1-kberserkerghostk-allocation-v6.service",
    "ultimatefish-info-opposed-ghost-pair-current-v3.service",
    "ultimatefish-info-parasite-opposed-resume-v5.service",
    "ultimatefish-devil-companion-v23-same-bishop-square-1.service",
)
SOURCES = (
    "/mnt/ultimatefish/info-substate-corrected-v1/kghostkchecker",
    "/mnt/ultimatefish/info-substate-corrected-v1/kberserkerghostk-v2-resume-v5",
    "/mnt/ultimatefish/opposed-ghost-pair-current-5b815-v1",
    "/mnt/ultimatefish/parasite-tracked-current-opposing-v1",
    "/mnt/ultimatefish/devil-companion-v15-34aa7f01/same-bishop/square-1",
)


def launch() -> str:
    unit = "ultimatefish-preserve-i024-resize-v1"
    receipt = f"{ROOT}/receipt.txt"
    log = f"{ROOT}/preserve.log"
    lines = ["set -euo pipefail", f"install -d -m 0700 {shlex.quote(ROOT)}"]
    ...
    lines.extend([
        f"printf '%s\\n' format=ultimatefish-i024-resize-preservation-v1 >{shlex.quote(receipt + '.stage')}",
        f"sha256sum {shlex.quote(ROOT)}/*.source.sha256 >>{shlex.quote(receipt + '.stage')}",
        f"mv {shlex.quote(receipt + '.stage')} {shlex.quote(receipt)}",
        f"sync -f {shlex.quote(ROOT)}",
        "printf '%s\\n' I024_RESIZE_CHECKPOINTS_AUTHENTICATED",
    ])
    command = "\n".join(lines) + f" >>{shlex.quote(log)} 2>&1"
    commands = [
        "set -euo pipefail",
        f"test ! -e {shlex.quote(receipt)}",
        "test $(df -B1 --output=avail /mnt/checkpoint-migrate | tail -1) -ge 900000000000",
        *[f"systemctl stop {shlex.quote(active)}" for active in UNITS],
        *[f"! systemctl is-active --quiet {shlex.quote(active)}" for active in UNITS],
        f"systemd-run --quiet --collect --unit={unit} --property=AllowedCPUs=24,25 --property=Nice=10 --property=MemoryMax=4294967296 /bin/bash -lc {shlex.quote(command)}",
...
def status() -> str:
    return remote.send(INSTANCE, [
        "systemctl show ultimatefish-preserve-i024-resize-v1.service -p ActiveState -p SubState -p Result -p ExecMainStatus --no-pager || true",
