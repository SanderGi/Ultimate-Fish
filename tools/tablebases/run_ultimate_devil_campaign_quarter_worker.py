#!/usr/bin/env python3
"""Split one Devil sixteenth-shard across disjoint campaign quarters."""
...
def main() -> None:
    ...
    parser.add_argument("--parent-campaign-lane", type=int, choices=(0, 1),
                        required=True)
    parser.add_argument("--campaign-residue", type=int, choices=range(4),
                        required=True)
    parser.add_argument("--shard-residue", type=int, choices=range(16),
                        required=True)
    ...
    if (base.sha256_path(args.plan) != args.plan_sha256 or
            ...
            len(args.prior_log) != 6 or len(args.prior_log_sha256) != 6 or
            any(base.sha256_path(path) != expected for path, expected in
                zip(args.prior_log, args.prior_log_sha256)) or
            args.campaign_residue % 2 != args.parent_campaign_lane):
        raise RuntimeError("Devil campaign-quarter provenance residual")
    ...
    if (workers != 133 or not any(
        ...
        raise RuntimeError("Devil campaign-quarter plan residual")
    campaigns = plan["pair_campaigns"]
    classes = {str(campaign["id"]) for campaign in campaigns}
    if len(classes) != len(campaigns):
        raise RuntimeError("duplicate Devil campaign-quarter class")
    ...
    marker = (f"DEVIL_CAMPAIGN_QUARTER_COMPLETE source_worker={args.source_worker} "
              f"campaign_residue={args.campaign_residue} "
              f"shard_residue={args.shard_residue} residual=0")
    if args.log.exists():
