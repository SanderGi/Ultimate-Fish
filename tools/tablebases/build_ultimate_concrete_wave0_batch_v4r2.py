...
import ultimate_concrete_wave0_batch_plan as plan  # noqa: E402
...
_BASE_DEFAULT_NAMESPACE = {
    "BATCH_VERSION": plan.BATCH_VERSION,
    "SCHEMA": plan.SCHEMA,
    "UNIT_PREFIX": plan.UNIT_PREFIX,
    "SOURCE_ROOT": plan.SOURCE_ROOT,
    "DEPENDENCY_BASE_ROOT": plan.DEPENDENCY_BASE_ROOT,
    "MANIFEST": plan.MANIFEST,
    "JOB_FRAGMENT": plan.JOB_FRAGMENT,
}
...
def _configure_base() -> None:
    ...
    plan.BATCH_VERSION = BATCH_VERSION
    plan.SCHEMA = SCHEMA
    plan.UNIT_PREFIX = UNIT_PREFIX
    plan.SOURCE_ROOT = SOURCE_ROOT
    plan.DEPENDENCY_BASE_ROOT = DEPENDENCY_BASE_ROOT
    plan.MANIFEST = MANIFEST
    plan.JOB_FRAGMENT = JOB_FRAGMENT
    plan._configure_base()


def _restore_base_namespace() -> None:
    for name, value in _BASE_DEFAULT_NAMESPACE.items():
...
def build_document(count: int = plan.DEFAULT_CLASSES) -> dict[str, object]:
    ...
    try:
        ...
        document["selection_policy"] = plan.SELECTION_POLICY
        ...
        document["scheduler_policy"] = {
            ...
            "assignment_order": list(plan.HOST_ASSIGNMENT_ORDER),
            "unassigned_host": "i-08c0f44a1776cb34a",
        }
        document["dependency_archive"] = dict(plan.DEPENDENCY_ARCHIVE)
        ...
        _restore_base_namespace()


def build_supervision_jobs(document: Mapping[str, object]) -> dict[str, object]:
    _configure_base()
    try:
        fragment = plan.base.build_supervision_jobs(document)
...
def merge_supervision_config(document: Mapping[str, object], path: Path = SUPERVISION_CONFIG) -> None:
    ...
    for raw in jobs:
        ...
        # explicit queue. Historical records outside this namespace are
        # retained verbatim.
    ...
    for raw in fragment["jobs"]:
        ...
        merged.append(job)
    current["jobs"] = merged
    plan.base.supervisor.validate_config(current)
    path.write_text(json.dumps(current, indent=2) + "\n")


def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--classes", type=int, default=plan.DEFAULT_CLASSES)
