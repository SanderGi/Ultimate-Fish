...
# diagnostic with the required extra-primary concrete codec specialization.
set -euo pipefail
...
readonly root=/mnt/ultimatefish/penguin-singleton-child-diagnostic-v32
readonly source=${root}/source.tar
readonly tree=${root}/source
readonly binary=${root}/ultimate_ghost_penguin_singleton_child_diagnostic_v32_linux
...
readonly binary_key="sources/binaries/penguin-singleton-child-diagnostic-v32/sha256/${binary_sha}/ultimate_ghost_penguin_singleton_child_diagnostic_v32_linux"
readonly binary_version="$(aws s3api put-object --region us-west-2 \
  --bucket "${bucket}" --key "${binary_key}" --body "${binary}" \
  --metadata "sha256=${binary_sha},source-sha256=${source_sha},purpose=singleton-child-wdl-diagnostic-extra-primary" \
...
cmp "${binary}" "${restored}"
printf '%s\n' \
  "PENGUIN_SINGLETON_CHILD_DIAGNOSTIC_V32_PRESERVED sha256=${binary_sha} size=${binary_size} version_id=${binary_version} key=${binary_key}"
