...
# diagnostic. The source is the exact v28 solver plus retained-root resume and
# additional failing-edge evidence; it does not regenerate transitions.
set -euo pipefail

readonly bucket=ultimatefish-info-20260808-a4e679c6-831688117652
readonly source_sha=ee309dd1f5a7fe89aabdadca95ed1030db5230312165fdba0bc11e1c295d05d0
readonly source_version=TCzSUBS52jeOgf2okpMLRRIKhtrdlz8H
readonly source_key="sources/bundles/penguin-singleton-child-diagnostic-v30/sha256/${source_sha}/ultimatefish-penguin-singleton-child-diagnostic-v30-source.tar"
readonly root=/mnt/ultimatefish/penguin-singleton-child-diagnostic-v30
readonly source=${root}/source.tar
readonly tree=${root}/source
readonly binary=${root}/ultimate_ghost_penguin_singleton_child_diagnostic_v30_linux
...
readonly binary_key="sources/binaries/penguin-singleton-child-diagnostic-v30/sha256/${binary_sha}/ultimate_ghost_penguin_singleton_child_diagnostic_v30_linux"
...
cmp "${binary}" "${restored}"
printf '%s\n' \
  "PENGUIN_SINGLETON_CHILD_DIAGNOSTIC_V30_PRESERVED sha256=${binary_sha} size=${binary_size} version_id=${binary_version} key=${binary_key}"
