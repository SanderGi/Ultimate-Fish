#!/usr/bin/env bash
# Build the same-side Pawn/Ghost transition graph from an empty tree using the
# self-tested adjacent-real-King-padding domain gate.
set -euo pipefail
...
work=/mnt/ultimatefish/info-substate-corrected-v1/kpawnghostk-fresh-v11

test ! -e "${work}"
test "$(sha256sum "${binary}" | cut -d' ' -f1)" = \
  9a7c8202e88ac0144e820409817db159343279d18c21a965c29bcaaf1bc89643
exec python3 "${runner}" \
  ...
  --prebuilt-executable-sha256 9a7c8202e88ac0144e820409817db159343279d18c21a965c29bcaaf1bc89643 \
