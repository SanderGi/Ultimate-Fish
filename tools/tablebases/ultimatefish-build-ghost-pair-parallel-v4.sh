...
readonly work=/mnt/ultimatefish/ghost-pair-parallel-v4-5a775115
readonly bundle="${work}/ultimatefish-ghost-pair-parallel-v4-source.tar.gz"
readonly bundle_key=sources/bundles/ghost-pair-parallel-v4/sha256/5a7751157d992769fb200b5ffb8e907cfad9ae1000c88396ab33e0a2ea72dcd6/ultimatefish-ghost-pair-parallel-v4-source.tar.gz
readonly bundle_version=jN6RxS2TgAplV1CkVRW147rEQFNVxwlJ
readonly source="${work}/source"
readonly binary="${work}/ultimate_ghost_pair_information_tablebase-parallel-v4"
readonly test_binary="${work}/ultimate_ghost_pair_information_solver_test-parallel-v4"
...
test "$(sha256sum "${bundle}" | cut -d ' ' -f 1)" = \
  5a7751157d992769fb200b5ffb8e907cfad9ae1000c88396ab33e0a2ea72dcd6
tar -xzf "${bundle}" -C "${source}"

cd "${source}"
sha256sum -c <(printf '%s  %s\n' \
  27b18ef2b5c4bfb5ecab68bffe9b2bfe454bc796ea9fc49ab5dfa8496543c0ea src/ultimate/tablebases/ghost_pair_information_solver.cpp \
...
readonly binary_key="sources/binaries/ghost-pair-parallel-v4/sha256/${binary_sha}/ultimate_ghost_pair_information_tablebase-parallel-v4"
readonly binary_version="$(aws s3api put-object --region us-west-2 \
  --bucket "${bucket}" --key "${binary_key}" --body "${binary}" \
  --metadata "sha256=${binary_sha},source-bundle-sha256=5a7751157d992769fb200b5ffb8e907cfad9ae1000c88396ab33e0a2ea72dcd6" \
  --query VersionId --output text)"
test -n "${binary_version}"
install -d -m 0755 "${work}/restore"
aws s3api get-object --region us-west-2 --bucket "${bucket}" \
  --key "${binary_key}" --version-id "${binary_version}" \
  "${work}/restore/ultimate_ghost_pair_information_tablebase-parallel-v4"
cmp -s "${binary}" \
  "${work}/restore/ultimate_ghost_pair_information_tablebase-parallel-v4"

python3 -c 'import json,pathlib,sys; pathlib.Path(sys.argv[1]).write_text(json.dumps({"schema":"ultimate-ghost-pair-parallel-build-v4","source_bundle":{"sha256":"5a7751157d992769fb200b5ffb8e907cfad9ae1000c88396ab33e0a2ea72dcd6","version_id":"jN6RxS2TgAplV1CkVRW147rEQFNVxwlJ"},"binary":{"sha256":sys.argv[2],"bytes":int(sys.argv[3]),"key":sys.argv[4],"version_id":sys.argv[5]},"concurrency_test_residual":0,"self_test_residual":0,"s3_restore_residual":0},indent=2,sort_keys=True)+"\\n")' \
