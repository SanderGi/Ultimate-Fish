...
readonly scratch=${root}/work/solve-current-v37/kghostkchecker
readonly old_log=${root}/work/logs/compositional-merge-solve-v4.log
readonly log=${root}/work/logs/checker-current-v37.log
readonly output=${root}/work/results/kghostkchecker-current-v37.ufiw
readonly arbitrary=${root}/work/results/kghostkchecker-current-v37.ufgd
...
exec >>"${log}" 2>&1
echo "checker_opposed_current_v37 source_sha256=${source_sha} normalized_sha256=${normalized_sha} retained_zero_change_iteration=26 workers=32"
...
"${binary}" --solve --orientation opposing \
  ...
  --resume-fixed-point --resume-converged --resume-iteration 26 \
  --resume-current-slot current --resume-bdd-slot a --workers 32

sha256sum "${output}" "${arbitrary}"
echo 'checker_opposed_current_v37 complete 1'
