#!/usr/bin/env bash
# Start the authenticated horizontal-only same-side Pawn/Ghost fixed point.
# This source generation predates the worker CLI; the exact serial solve is a
# safe fallback while its Bellman implementation is being parallelized.
set -euo pipefail

readonly root=/mnt/ultimatefish/info-substate-corrected-v1/kpawnghostk-fresh-v12
readonly binary=${root}/ultimate_ghost_ordinary_information_tablebase
readonly stem=kpawnghostk
...
test ! -e "${root}/work/results/${stem}.ufiw"
test ! -e "${root}/work/results/${stem}.ufgd"

exec >>"${root}/work/logs/solve-v15.log" 2>&1
echo 'pawn_ghost_horizontal_solve_v15 exact_transition_certificate 1 promoted_queen_ghost_current 1 serial_fallback 1'
cd "${root}"
exec "${binary}" --solve \
  --orientation same \
  --transition-prefix "work/transitions/${stem}" \
  --input "tablebases/${stem}.uftb" \
  --lower-ghost-sidecar tablebases/kghostk.ufgm \
  --scratch "work/solve/${stem}" \
  --output "work/results/${stem}.ufiw" \
  --output-arbitrary "work/results/${stem}.ufgd" \
