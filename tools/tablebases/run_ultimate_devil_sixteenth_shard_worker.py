#!/usr/bin/env python3
"""Resume one stopped Devil eighth-shard as disjoint sixteenth-shards."""
...
def main() -> None:
    ...
    parser.add_argument("--campaign-lane", type=int, choices=(0, 1),
                        required=True)
    parser.add_argument("--parent-residue", type=int, choices=range(8),
                        required=True)
    ...
    if (base.sha256_path(args.plan) != args.plan_sha256 or
            ...
            len(args.prior_log) != 5 or len(args.prior_log_sha256) != 5 or
            any(base.sha256_path(path) != expected for path, expected in
                zip(args.prior_log, args.prior_log_sha256)) or
            args.shard_residue % 8 != args.parent_residue):
        raise RuntimeError("Devil sixteenth-shard provenance residual")
    ...
    if (workers != 133 or not any(
        ...
        raise RuntimeError("Devil sixteenth-shard plan residual")
    campaigns = plan["pair_campaigns"]
    classes = {str(campaign["id"]) for campaign in campaigns}
    if len(classes) != len(campaigns):
        raise RuntimeError("duplicate Devil sixteenth-shard class")
    ...
    marker = (f"DEVIL_SIXTEENTH_SHARD_COMPLETE source_worker={args.source_worker} "
