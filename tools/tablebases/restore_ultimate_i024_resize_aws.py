#!/usr/bin/env python3
"""Restore and authenticate i-024 scratch payloads after its resize."""
...
import preserve_ultimate_i024_resize_aws as preserve


DESTINATIONS = preserve.SOURCES[:4]


def launch() -> str:
    unit = "ultimatefish-restore-i024-resize-v1"
    ...
    lines.extend([
        f"printf '%s\\n' format=ultimatefish-i024-resize-restore-v1 >{shlex.quote(receipt + '.stage')}",
        *[f"sha256sum {shlex.quote(preserve.ROOT + f'/payload-{number}.restored.sha256')} >>{shlex.quote(receipt + '.stage')}" for number in range(len(DESTINATIONS))],
        f"mv {shlex.quote(receipt + '.stage')} {shlex.quote(receipt)}",
        "sync -f /mnt/ultimatefish",
        "printf '%s\\n' I024_RESIZE_RESTORE_AUTHENTICATED",
    ])
    command = "\n".join(lines) + f" >>{shlex.quote(log)} 2>&1"
    commands = [
        "set -euo pipefail",
        "test $(findmnt -n -o TARGET /mnt/ultimatefish) = /mnt/ultimatefish",
        f"test ! -e {shlex.quote(receipt)}",
        f"systemd-run --quiet --collect --unit={unit} --property=AllowedCPUs=48-51 --property=Nice=10 --property=MemoryMax=4294967296 /bin/bash -lc {shlex.quote(command)}",
...
def status() -> str:
    return remote.send(preserve.INSTANCE, [
        "systemctl show ultimatefish-restore-i024-resize-v1.service -p ActiveState -p SubState -p Result -p ExecMainStatus --no-pager || true",
