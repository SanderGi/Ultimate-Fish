#!/usr/bin/env python3
"""Launch authenticated, disjoint Devil campaign-split v4 workers."""
...
def main() -> None:
    ...
    if sha256_path(args.manifest) != args.manifest_sha256:
        raise RuntimeError("Devil campaign-split manifest hash residual")
    manifest = json.loads(args.manifest.read_text())
    if (manifest["schema"] != "ultimate-devil-campaign-split-v4" or
            ...
            manifest["campaign_modulus"] != 2):
        raise RuntimeError("Devil campaign-split manifest binding residual")
    ...
    for row in manifest["workers"]:
        ...
        old_unit = f"ultimatefish-devil-fleet-v6-repartition-v2-{worker}.service"
        if subprocess.run(["systemctl", "is-active", "--quiet", old_unit],
                          check=False).returncode == 0:
            raise RuntimeError("Devil source worker is still active")
        prior = args.root / "fleet-repartition-v2" / f"worker-{worker}.log"
        if sha256_path(prior) != row["prior_log_sha256"]:
            raise RuntimeError("Devil campaign-split prior-log residual")
        logs[worker] = prior
        for raw_lane, raw_cpu in row["lane_cpus"].items():
            ...
            if lane not in (0, 1) or cpu in targets or cpu in occupied:
                raise RuntimeError("Devil campaign-split CPU allocation residual")
            targets.add(cpu)
    ...
    for row in manifest["workers"]:
        ...
        for raw_lane, raw_cpu in row["lane_cpus"].items():
            lane, cpu = int(raw_lane), int(raw_cpu)
            unit = f"ultimatefish-devil-fleet-v6-campaign-split-v4-{worker}-{lane}.service"
            log = args.root / "fleet-campaign-split-v4" / f"worker-{worker}-lane-{lane}.log"
            subprocess.run([
                ...
                "--lane", str(lane),
                "--prior-log", str(logs[worker]),
                "--prior-log-sha256", row["prior_log_sha256"],
                "--log", str(log),
                "--bucket", args.bucket,
                "--receipt-prefix", manifest["receipt_prefix"],
            ], check=True)
    print(f"DEVIL_CAMPAIGN_SPLIT_V4_LAUNCHED workers={len(targets)} residual=0")
