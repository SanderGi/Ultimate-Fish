#!/usr/bin/env bash
# Fresh same-side Ghost/Ghost sprint solve over the already authenticated graph.
# The stopped v2 solver has no fixed-point resume interface, so its partial
# roots are hash-preserved before a new, non-overlapping solve is started.
set -euo pipefail
...
readonly evidence=${root}/retained-partial-solve-v2-20260828T1455Z
...
if [[ -d "${work}/work/solve" ]]; then
  test ! -e "${evidence}"
  mkdir -p "${evidence}"
  find "${work}/work/solve" -maxdepth 1 -type f -print0 \
    | sort -z | xargs -0 sha256sum >"${evidence}/partial-solve.sha256"
  find "${work}/work/solve" -maxdepth 1 -type f -printf '%s %f\n' \
    | sort -k2 >"${evidence}/partial-solve.sizes"
  mv "${work}/work/solve" "${evidence}/solve"
  if [[ -s "${work}/work/logs/solve.log" ]]; then
    mv "${work}/work/logs/solve.log" "${evidence}/solve.log"
  fi
fi

test -d "${evidence}/solve"
test -s "${evidence}/partial-solve.sha256"
test -s "${evidence}/partial-solve.sizes"
