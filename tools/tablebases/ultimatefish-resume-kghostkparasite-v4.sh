#!/usr/bin/env bash
# Resume opposed Parasite/Ghost from authenticated compaction 6 after a clean,
# byte-identical remerge restored the transition bundle. Bdd-a is append-only,
# so the partial iteration-7 suffix does not alter retained compaction-6 node
# IDs; partial next roots are ignored.
set -euo pipefail
...
exec >>"${log}" 2>&1
echo 'parasite_opposed_resume_v4 resume_iteration=6 current_slot=current bdd_slot=a append_only_partial_iteration=7 transition_remerge_v2_residual=0'
cd "${root}"
exec python3 -c '
import json
import os
