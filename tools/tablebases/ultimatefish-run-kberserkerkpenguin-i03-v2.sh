#!/usr/bin/env bash
# Retry opposing K+Berserker-v-K+Penguin after v1 exceeded its 64-GiB RSS gate.
set -euo pipefail

bucket=ultimatefish-info-20260808-a4e679c6-831688117652
root=/mnt/ultimatefish/berserker-penguin-opposed-v2
source_root=$root/source
dependencies=$root/dependencies
work=$root/work

test ! -e "$root"
test -s /mnt/ultimatefish/berserker-penguin-opposed-v1/work/logs/generate/kberserkerkpenguin.log
...
exec /usr/bin/python3 \
  ...
  --resident-limit 171798691840 \
  --reverse-edge-bytes-limit 858993459200 \
  --minimum-free-bytes 214748364800 \
  --workers 8 \
  --s3-prefix results/current-concrete-d64b5cdf/berserker-penguin-opposed-v2
