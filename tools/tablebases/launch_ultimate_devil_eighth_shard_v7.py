#!/usr/bin/env python3
"""Launch authenticated, disjoint Devil eighth-shard v7 workers."""
...
def main() -> None:
    ...
    if sha256_path(args.manifest) != args.manifest_sha256:
        raise RuntimeError("Devil eighth-shard manifest hash residual")
    manifest = json.loads(args.manifest.read_text())
    if (manifest["schema"] != "ultimate-devil-eighth-shard-v7" or
            ...
            manifest["shard_modulus"] != 8):
        raise RuntimeError("Devil eighth-shard manifest binding residual")
    ...
    for row in manifest["lanes"]:
        ...
        old_unit = ("ultimatefish-devil-fleet-v6-quarter-shard-v6-"
                    f"{worker}-{campaign}-{parent}.service")
        if subprocess.run(["systemctl", "is-active", "--quiet", old_unit],
                          check=False).returncode == 0:
            raise RuntimeError("Devil source quarter-shard is still active")
        prior_shard = parent % 2
        prior_paths = (
        ...
        if [sha256_path(path) for path in prior_paths] != row["prior_hashes"]:
            raise RuntimeError("Devil eighth-shard prior-log residual")
        for raw_residue, raw_cpu in row["residue_cpus"].items():
            residue, cpu = int(raw_residue), int(raw_cpu)
            if (campaign not in (0, 1) or parent not in range(4) or
                    residue not in range(8) or residue % 4 != parent or
                    cpu in targets or cpu in occupied):
                raise RuntimeError("Devil eighth-shard CPU residual")
            targets.add(cpu)
    ...
    for row in manifest["lanes"]:
        ...
        for raw_residue, raw_cpu in row["residue_cpus"].items():
            residue, cpu = int(raw_residue), int(raw_cpu)
            unit = ("ultimatefish-devil-fleet-v6-eighth-shard-v7-"
                    f"{worker}-{campaign}-{residue}.service")
            log = (args.root / "fleet-eighth-shard-v7" /
