...
unit=ultimatefish-info-kghostghostk-transitions-after-stop-v1.service
root=/mnt/ultimatefish/same-ghost-pair-rebuild-after-stop-v1
migration=$root/migration
marker=$root/job/work/transitions-ready.json
archive=$migration/kghostghostk-transitions-v1.tar.zst
...
key="results/migrations/same-ghost-pair-transitions-v1/sha256/$archive_sha/kghostghostk-transitions-v1.tar.zst"
...
/usr/bin/python3 -c 'import hashlib,json,pathlib,sys; migration=pathlib.Path(sys.argv[1]); marker=pathlib.Path(sys.argv[2]); key=sys.argv[3]; sha=sys.argv[4]; size=int(sys.argv[5]); head=json.loads((migration/"archive-head.json").read_text()); assert head["ContentLength"] == size and head["Metadata"]["sha256"] == sha; receipt={"schema":"ultimate-ghost-pair-transition-migration-v1","filename":"kghostghostk.uftb","archive":{"bucket":"ultimatefish-info-20260808-a4e679c6-831688117652","key":key,"version_id":head["VersionId"],"bytes":size,"sha256":sha},"transition_marker":{"bytes":marker.stat().st_size,"sha256":hashlib.sha256(marker.read_bytes()).hexdigest()},"source_sha256":"12e053d81f0594d363830db97eb71a87a27a73fc3de0dcfe8cb13f141bdda8e6","model_sha256":"779e91a7df7dc806a472963b1073470d484e7a1b638d71dfc9c8dbe5c839e110","observation_sha256":"890c399d6856fbf1773766f1840d0c38e46669c18b609e20c38d2750a09dbc23"}; (migration/"migration-ready.json").write_text(json.dumps(receipt,indent=2,sort_keys=True)+"\n")' "$migration" "$marker" "$key" "$archive_sha" "$archive_bytes"
