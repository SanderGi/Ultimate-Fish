...
for path in "${transition}.verified" "${scratch}.bdd-a.nodes" \
  "${scratch}.bdd-a.unique" "${scratch}.domains" \
...
# Iteration 10 and its compaction are journal-authenticated.  Physical current
# and ROBDD a are the exact roots.  Iteration-11 nodes are append-only reusable
# cache entries.  Node arenas can be sparse-extended; the disposable unique
# indices must retain their old size so the v12 binary rebuilds them exactly
# under the larger hash mask instead of trusting an invalid zero-padded table.
for slot in a b; do
  nodes="${scratch}.bdd-${slot}.nodes"
  unique="${scratch}.bdd-${slot}.unique"
  test "$(stat -c %s "${nodes}")" = 18000000000 -o \
    "$(stat -c %s "${nodes}")" = 27000000000
...
printf '%s\n' 'iteration=10' 'current_slot=current' 'bdd_slot=a' \
  'max_nodes=3000000000' 'unique_slots=4294967296' \
  "source_bundle_sha256=${source_sha}" "source_binary_sha256=${binary_sha}" \
  >"${scratch}.action-conditioned-v15"
...
exec "${binary}" --solve --orientation opposing \
  ...
  --max-nodes 3000000000 --unique-slots 4294967296 \
  --compact-every 1 --resume-fixed-point --resume-iteration 10 \
