#!/usr/bin/env python3
"""Build deterministic AWS bundles for exact Fisherman/Ghost information."""
...
BUILD_INPUTS_COMMON = (
    "src/ultimate/tablebases/ghost_fisherman_information_tablebase.cpp",
    "src/ultimate/tablebases/ghost_fisherman_information_solver.cpp",
    "src/ultimate/tablebases/ghost_fisherman_information_solver.h",
    ...
    "tools/tablebases/run_ultimate_ghost_fisherman_aws.py",
)
TEXTUAL_CPP = {
    "src/ultimate/tablebases/ghost_public_extra_information_solver.cpp",
    "src/ultimate/tablebases/ghost_extra_information_tablebase.cpp",
...
def shard_ranges() -> list[tuple[str, int, int]]:
    ...
    if cursor != GEOMETRIES:
        raise RuntimeError("balanced Fisherman/Ghost shard arithmetic residual")
    return ranges
...
def build_manifest(filename: str) -> dict[str, object]:
    ...
    executable = "./ultimate_ghost_fisherman_information_tablebase"
    ...
    solve_arguments = [
        ...
        "--output-arbitrary", f"work/results/{stem}.ufgf",
    ...
    artifacts = [
        *(f"{transition}.{suffix}" for suffix in
        ...
        f"work/results/{stem}.ufiw", f"work/results/{stem}.ufgf",
    ...
    return {
        "schema": "ultimate-fisherman-ghost-aws-v1",
