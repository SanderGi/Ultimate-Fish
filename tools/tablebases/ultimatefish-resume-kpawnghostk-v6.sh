#!/bin/bash
# Resume same Pawn/Ghost from the authenticated compacted iteration-42 roots.
# V6 adds collision-checked reopening of the dense persisted ROBDD prefix;
# iteration 43 touched only alternate arrays and remains ignored.
set -euo pipefail

root=/mnt/ultimatefish/info-substate-corrected-v1/kpawnghostk-v4
binary=/mnt/ultimatefish/pawn-resume-v6-7136049d/ultimate_ghost_ordinary_information_tablebase-pawn-resume-v6
...
exec >>"${log}" 2>&1
echo 'pawn_ghost_same_resume_v6 resume_iteration 42 current_slot current bdd_slot a checkpoint_migration 1 base_source_sha256 4be43c8bf7c19f636cc051fc77a7d1496dd263ecb62f5d447a1e83a8ba646e35 correction_sha256 da563e009e78d8b5d6b2df670471debbf04751a7ba8548cc1bec27b2fb53b654 resume_patch_sha256 25899b617a89f7aa63a965d4c30a78563bb298a087c1f9bc4e0735f3ea3295c8 open_patch_sha256 7136049d11357523571c3d1c36660a4adcd89405700bba94b998272d702ff4af binary_sha256 2a25f6a0ff36b8882f53e9917a6dab8a45b3c73377a112cf6687bb2292039498'
cd "${root}"
"${binary}" --solve \
  --orientation same \
