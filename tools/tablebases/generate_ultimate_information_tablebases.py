...
def _empty_document() -> dict[str, object]:
    # Checkpoint schemas and source fingerprints are repository metadata.  They
    # remain inspectable after the large payloads have been archived to S3.
    # Individual solve commands still authenticate their required local input
    # immediately before use.
    records = information.affected_inventory(require_files=False)
    return {
...
def arbitrary_is_current(path: Path, source_sha256: str,
                         ...
                         parasite_orientation: int | None = None,
                         giant_orientation: int | None = None) -> bool:
    ...
    try:
        ...
        if magic == b"UFGG1\0\0\0":
            ...
            lower_bomb_sha = information.concrete_dependency_sha256(
                "kbombk.uftb")
            dependencies_current = (
                header[352:416].decode() ==
                    information.observation_model_fingerprint() and
        ...
        if magic == b"UFGD1\0\0\0":
            ...
            material_current = (
                struct.unpack_from("<I", header, 20)[0] ==
                    PIECE_TYPE_IDS["ghost"] and
                struct.unpack_from("<I", header, 24)[0] ==
                    PIECE_TYPE_IDS["giant"] and
                struct.unpack_from("<I", header, 28)[0] == 0 and
                struct.unpack_from("<I", header, 32)[0] in {0, 1} and
                (giant_orientation is None or
                 struct.unpack_from("<I", header, 32)[0] ==
                    giant_orientation))
        return (
            header[:8] == magic and
...
def solver_command(args: argparse.Namespace, record: Mapping[str, object],
    ...
    if domain in {"giant-ghost-same", "giant-ghost-opposing"}:
        ...
        transitions = (args.giant_ghost_same_transitions
                       if orientation == "same"
                       else args.giant_ghost_opposing_transitions)
        lower_giant = ROOT / "tablebases" / "kgiantk.uftb"
        lower_giant_sha = shards.logical_sha256(lower_giant)
        return [
            str(args.giant_ghost_binary), "--solve",
            "--orientation", orientation,
            "--transition-prefix", str(transitions),
            "--input", str(table), "--lower-ghost-sidecar", str(lower),
            "--lower-giant-table", str(lower_giant),
            "--scratch", str(args.scratch / f"{Path(table).stem}-exact"),
            "--output", str(overlay), "--output-arbitrary",
            str(args.overlays / f"{Path(table).stem}.ufgi"),
            "--source-sha256", source_sha256,
            "--model-sha256", model_sha256,
            "--observation-sha256",
            information.observation_model_fingerprint(),
            ...
            "--lower-model-sha256", payload[160:224].decode(),
            "--lower-observation-sha256", payload[224:288].decode(),
            "--lower-giant-sha256", lower_giant_sha,
            "--lower-giant-source-sha256", lower_giant_sha,
            "--lower-giant-model-sha256",
            information.concrete_tablebase_model_fingerprint("kgiantk.uftb"),
            "--compact-every", "1",
        ]
...
def solve_one(args: argparse.Namespace) -> None:
    ...
    arbitrary = (args.overlays / "kghostghostk.ufgg" if domain == "ghost-pair"
                 ...
                 else args.overlays / f"{Path(args.filename).stem}.ufgi"
                 if domain in {"giant-ghost-same",
    ...
    giant_orientation = (0 if domain == "giant-ghost-same" else
                         1 if domain == "giant-ghost-opposing" else None)
    if domain == "jester-ghost":
    ...
    if (isinstance(entry, dict) and
            ...
            (domain not in {"ghost-pair", "jester-ghost",
                            ...
                            "giant-ghost-opposing"} or arbitrary_is_current(
                ...
                giant_orientation=giant_orientation))):
    ...
    if domain in {"ghost-pair", "jester-ghost",
                  ...
                  "mage-ghost-opposing", "parasite-ghost-same",
                  "parasite-ghost-opposing", "giant-ghost-same",
                  "giant-ghost-opposing"} and not arbitrary_is_current(
            arbitrary, source_sha256, model_sha256,
            lower_jester_overlay_sha256=lower_jester_arbitrary_sha,
            dragon_orientation=dragon_orientation,
            fisherman_orientation=fisherman_orientation,
...
def main() -> None:
    ...
    parser.add_argument("--giant-ghost-binary", type=Path,
                        default=ROOT / "src" /
                        "ultimate_ghost_giant_information_tablebase")
    parser.add_argument("--giant-ghost-same-transitions", type=Path,
                        default=Path(
                          "/tmp/kghostgiantk-exact-transitions"))
    parser.add_argument("--giant-ghost-opposing-transitions", type=Path,
                        default=Path(
                          "/tmp/kghostkgiant-exact-transitions"))
