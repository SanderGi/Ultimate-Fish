#!/usr/bin/env python3
"""Resume retained Devil checkpoints with phase-advised v21 key I/O."""
...
import resume_ultimate_devil_companion_solver_v19_aws as v19


SOURCE_SHA256 = "dfe20d51bc1a8a3caf4d4bb74eb79119ff4731e2a084e34d097bf3b4826031d3"
SOURCE_VERSION = "UewGSNkdUreKcD84onUvq.QiB_GOa91n"
SOURCE_KEY = (
    f"sources/bundles/devil-spawned-solver-v21/sha256/{SOURCE_SHA256}/"
    "ultimatefish-devil-spawned-solver-v21-source.tar"
)
BINARY_SHA256 = "2a4c0b9330f368a54f19e4deb8a7de3e305c3f352268036d4101efee01f6ba4f"
BINARY_VERSION = "U3OdWUzXzb5XDamAVmMcNSaSyy6c742L"
BINARY_KEY = (
    f"sources/binaries/devil-spawned-solver-v21/sha256/{BINARY_SHA256}/"
    "ultimate_tablebase-devil-spawned-v21"
)
ROOT = f"/mnt/ultimatefish/devil-spawned-solver-v21-{SOURCE_SHA256[:8]}"
MEMORY_MAX = 240_518_168_576
MEMORY_HIGH = MEMORY_MAX
REVIEWED_MIN_HASH_CAPACITY = v19.REVIEWED_MIN_HASH_CAPACITY


def stage_commands() -> list[str]:
    source = f"{ROOT}/source.tar"
    binary = f"{ROOT}/ultimate_tablebase-devil-spawned-v21"
    return [
...
def names(piece: str, opposed: bool, square: int,
    ...
    unit = f"ultimatefish-devil-companion-v21-{orientation}-{piece}-square-{square}"
    log = (f"{volume.rstrip('/')}/devil-companion-v15-34aa7f01/logs/"
           f"{orientation}-{piece}-square-{square}-v21.log")
    old_unit = f"ultimatefish-devil-companion-v20-{orientation}-{piece}-square-{square}"
...
def resume(instance: str, piece: str, opposed: bool, square: int,
    ...
    if (not cpus or limit <= 0 or limit >= 2**63 or
        ...
        raise ValueError("invalid v21 Devil resource")
    ...
    if reviewed_floor is not None and hash_capacity < reviewed_floor:
        raise ValueError(
            f"reviewed v21 hash capacity for {piece} square {square} is "
