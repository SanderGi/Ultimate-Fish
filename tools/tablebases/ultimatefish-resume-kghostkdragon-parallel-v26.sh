...
readonly log=work/logs/verify-solve-parallel-v26.log
...
for path in "${restore}/work/transitions/kghostkdragon.verified" \
  "${restore}/${scratch}.bdd-a.nodes" \
  "${restore}/${scratch}.bdd-a.unique" \
...
echo 'dragon_opposed_parallel_v26 resume_iteration=29 current_slot=next bdd_slot=a workers=32 checkpoint_residual=0'
cd "${restore}"
