#!/usr/bin/env python3
"""Resume one stopped Devil quarter-shard as disjoint eighth-shards."""
...
def main() -> None:
    ...
    parser.add_argument("--campaign-lane", type=int, choices=(0, 1),
                        required=True)
    parser.add_argument("--parent-residue", type=int, choices=range(4),
                        required=True)
    parser.add_argument("--shard-residue", type=int, choices=range(8),
                        required=True)
    ...
    if (base.sha256_path(args.plan) != args.plan_sha256 or
            ...
            len(args.prior_log) != 4 or len(args.prior_log_sha256) != 4 or
            any(base.sha256_path(path) != expected for path, expected in
                zip(args.prior_log, args.prior_log_sha256)) or
            args.shard_residue % 4 != args.parent_residue):
        raise RuntimeError("Devil eighth-shard provenance residual")
    ...
    marker = (f"DEVIL_EIGHTH_SHARD_COMPLETE source_worker={args.source_worker} "
    ...
    with args.log.open("a", encoding="utf-8") as output:
        for index, campaign in enumerate(campaigns):
            ...
            for ordinal, shard in enumerate(range(
                ...
                if ordinal % 8 != args.shard_residue:
                    continue
