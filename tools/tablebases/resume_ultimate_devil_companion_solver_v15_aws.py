#!/usr/bin/env python3
"""Resume a migrated seven-byte-key Devil companion checkpoint under v15."""
...
import migrate_ultimate_devil_companion_v15_aws as migration
...
def names(piece: str, opposed: bool, square: int,
          volume: str) -> tuple[str, ...]:
    orientation = "opposed" if opposed else "same"
    root = f"{volume.rstrip('/')}/devil-companion-v15-{migration.SOURCE_SHA256[:8]}"
    work = f"{root}/{orientation}-{piece}/square-{square}"
    unit = f"ultimatefish-devil-companion-v15-{orientation}-{piece}-square-{square}"
    log = f"{root}/logs/{orientation}-{piece}-square-{square}.log"
    source_unit, _, _ = v11.names(piece, opposed, square, volume)
    v14_unit, _, _, _ = v14.names(piece, opposed, square, volume)
    return unit, work, log, source_unit, v14_unit
...
def resume(instance: str, piece: str, opposed: bool, square: int,
    ...
    if piece not in v11.STATELESS or square not in base.FIXED_SQUARES:
        raise ValueError("invalid v15 companion resume material")
    if not cpus or len(set(cpus)) != len(cpus):
        raise ValueError("v15 companion CPU allocation must be nonempty and unique")
    if (limit <= 0 or limit >= 2**63 or
            not (volume == "/mnt/checkpoint-migrate" or
                 volume.startswith("/mnt/ultimatefish"))):
        raise ValueError("invalid v15 companion resume resource")
    unit, work, log, source_unit, v14_unit = names(
        piece, opposed, square, volume)
    binary = f"{migration.ROOT}/ultimate_tablebase-devil-spawned-v15"
    ...
    commands = [
        ...
        *migration.stage_commands(),
        ...
        f"! systemctl is-active --quiet {v14_unit}.service",
...
def status(instance: str, piece: str, opposed: bool, square: int,
           volume: str) -> str:
    unit, work, log, _, v14_unit = names(piece, opposed, square, volume)
    return base.send(instance, [
        f"systemctl status {unit}.service --no-pager -l || true",
        f"systemctl show {v14_unit}.service -p ActiveState -p SubState "
...
def profile(instance: str, piece: str, opposed: bool, square: int,
            volume: str) -> str:
    unit, _, _, _, _ = names(piece, opposed, square, volume)
    return base.send(instance, [
        "set -euo pipefail",
...
def main() -> None:
    ...
    parser.add_argument("--volume", default=migration.DEFAULT_VOLUME)
