#!/usr/bin/env python3
"""Resume one stopped Devil half-shard lane as disjoint quarter lanes."""
...
def main() -> None:
    ...
    parser.add_argument("--campaign-lane", type=int, choices=(0, 1),
                        required=True)
    parser.add_argument("--prior-shard-lane", type=int, choices=(0, 1),
                        required=True)
    parser.add_argument("--shard-residue", type=int, choices=(0, 1, 2, 3),
                        required=True)
    ...
    if (base.sha256_path(args.plan) != args.plan_sha256 or
            ...
            len(args.prior_log) != 3 or len(args.prior_log_sha256) != 3 or
            any(base.sha256_path(path) != expected for path, expected in
                zip(args.prior_log, args.prior_log_sha256)) or
            args.shard_residue % 2 != args.prior_shard_lane):
        raise RuntimeError("Devil quarter-shard provenance residual")
    ...
    if (workers != 133 or not any(
        ...
        raise RuntimeError("Devil quarter-shard plan residual")
    campaigns = plan["pair_campaigns"]
    classes = {str(campaign["id"]) for campaign in campaigns}
    if len(classes) != len(campaigns):
        raise RuntimeError("duplicate Devil quarter-shard class")
    ...
    marker = (f"DEVIL_QUARTER_SHARD_COMPLETE source_worker={args.source_worker} "
    ...
    with args.log.open("a", encoding="utf-8") as output:
        for index, campaign in enumerate(campaigns):
            ...
            for ordinal, shard in enumerate(range(
                ...
                if ordinal % 4 != args.shard_residue:
                    continue
