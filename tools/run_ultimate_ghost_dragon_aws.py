#!/usr/bin/env python3
"""Run one authenticated exact Dragon/Ghost AWS bundle."""
...
def validate_manifest(manifest: dict[str, object]) -> None:
    ...
    if (manifest.get("schema") != "ultimate-dragon-ghost-aws-v1" or
        ...
        raise RuntimeError("invalid Dragon/Ghost manifest")
    shards = manifest.get("commands", {}).get("shards")
    if not isinstance(shards, list) or len(shards) != 64:
        raise RuntimeError("invalid Dragon/Ghost shard inventory")
...
def main() -> None:
    ...
    for key in ("schema", "filename", "orientation",
                "normalized_source_sha256", "lower_dragon_full_sha256",
                "lower_dragon_source_sha256",
