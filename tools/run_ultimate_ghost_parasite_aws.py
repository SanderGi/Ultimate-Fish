#!/usr/bin/env python3
"""Run one authenticated exact Parasite/Ghost AWS bundle."""
...
def validate_manifest(manifest: dict[str, object]) -> None:
    expected_orientation = {
        "kghostparasitek.uftb": "same",
        "kghostkparasite.uftb": "opposing",
    }.get(manifest.get("filename"))
    if (manifest.get("schema") != "ultimate-parasite-ghost-aws-v1" or
        ...
        raise RuntimeError("invalid Parasite/Ghost manifest")
    shards = manifest.get("commands", {}).get("shards")
    if not isinstance(shards, list) or len(shards) != 64:
        raise RuntimeError("invalid Parasite/Ghost shard inventory")
